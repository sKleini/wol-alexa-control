...
/**
 * **Wie lange ein Abruf hoechstens an der Box haengen darf.**
 *
 ...
 *
 * Zweiunddreissig Kilobyte in dreieinhalb Minuten - hundertsechzig Byte je
 * Sekunde. Die Box war verstopft, und der Durchleiter zog weiter an ihr: Der
 * Aufrufer (die Pruefung im Dashboard) hatte nach sieben Sekunden aufgegeben,
 * hier lief es noch hundertachtundneunzig Sekunden laenger. Er hat die Box
 * nicht entlastet, er hat sie festgehalten - und als der naechste Titel
 * gebraucht wurde, war sie immer noch dicht.
 *
 * `KOPF_MS` bewacht nur, **ob** die Box antwortet. Diese Frist bewacht, ob sie
 * fertig wird. Auf die Plattform ist dabei kein Verlass: `vercel.json` setzt
 * `maxDuration: 60`, und dieser Abruf lief 205 Sekunden.
 *
 * **Warum das mehr ist als Sparsamkeit.** Ein schneller Fehlschlag ist
 * heilbar, ein Haenger nicht: Auf ein 502 meldet Alexa `PlaybackFailed`, und
 * `nachFehler` wiederholt einmal und geht sonst weiter. Auf einen Haenger
 * meldet sie gar nichts, und die Kette - die seit #121 allein an
 * `PlaybackFinished` haengt - stirbt lautlos. Die Frist verwandelt also die
 * unheilbare Stille in einen Fall, den der Skill schon behandeln kann.
 *
 * Dreissig Sekunden: Ein Kapitel von 17,7 MB brauchte gemessen 18,1 s.
 */
// Die Untergrenze schuetzt nur vor einem Vertipper in der Umgebung; wer
// eine Sekunde einstellt, meint es so.
const FRIST_MS = () => Math.max(1000, Number(process.env.MUSIK_TON_FRIST_MS) || 30000);
...
async function tonLiefern(req, res, redis, rohToken, holeSid) {
  ...
  for (const erzwingen of [false, true]) {
    ...
    // Ab hier laeuft die Frist: Wer bis dahin nicht fertig ist, wird
    // abgebrochen - Function frei, Box frei. Siehe `FRIST_MS`.
    const endet = begonnen + FRIST_MS();
    ...
    try {
      lauf = await durchleiten(antwort.body, res, {
        ...
        // Die Frist ist strenger als die Uhr der Function - es gilt die
        // naehere der beiden Grenzen.
        nochZeit: () => Math.min(endet, begonnen + UHR_MS()) - Date.now() > ANLAUF_RESERVE_MS,
        pfad: auftrag.pfad,
        endet,
      });
    } finally {
    ...
    // **Der Eintrag, der die naechste Stoerung erklaert.** `bytes` gegen
    // `soll` sagt, ob der Strom vollstaendig war; `dauer` nahe 60000 zeigt
    // auf die Uhr der Function; `angemeldet` auf die Box-Anmeldung.
    await verlaufSchreiben(redis, {
      was: 'ton', datei: adresseKurz(auftrag.pfad), bereich: teil.kopfzeile, status,
      bytes: lauf.bytes, soll, dauer: Date.now() - begonnen, anlaeufe: lauf.anlaeufe, angemeldet,
      aufgegeben: lauf.aufgegeben,
    });
    if (lauf.aufgegeben) {
      console.warn(`musik-box Lieferung aufgegeben: ${lesbareMenge(lauf.bytes)}`
        + `${soll ? ` von ${lesbareMenge(soll)}` : ''} in ${Date.now() - begonnen} ms`
        + ` – die Box wurde nicht fertig, ${FRIST_MS()} ms sind die Grenze.`
        + ' Abgebrochen ist heilbar, haengen nicht: Alexa meldet jetzt einen Fehler.');
    } else if (soll && lauf.bytes < soll) {
 ...
 */
function durchleiten(koerper, res, { soll = null, holeRest = null, nochZeit = () => false, pfad = '', endet = Infinity } = {}) {
  return new Promise(async (fertig) => {
    ...
    // **Der Wachhund auf die Lieferung.** Er zerstoert den Lesestrom, und
    // damit bricht auch der Abruf bei der Box ab - das ist der Punkt: Eine
    // verstopfte Box wird freigegeben, statt weiter festgehalten zu werden.
    const wachhund = Number.isFinite(endet)
      ? setTimeout(() => { aufgegeben = true; strom?.destroy(); }, Math.max(0, endet - Date.now()))
      : null;
    ...
    await hinaus;
    fertig({ bytes, anlaeufe, aufgegeben });
  });
}
