 ...
 *
 * Solange geschrieben wird, steht die Lieferung auf dieser Liste - und frischt
 * ihren Eintrag jede Minute auf (`laeuftAuffrischen`). Wer dort einen
 * gueltigen Eintrag sieht, nimmt die gemerkte Nummer, auch wenn ihr Fenster
 * abgelaufen ist, und meldet sich nicht an. Die Frist je Eintrag ist die
 * Sicherung fuer den Fall, dass eine Function mitten im Schreiben stirbt:
 * Dann steht ihr Eintrag hoechstens zwei Minuten laenger, als er sollte.
 *
 * **Eine Liste, kein einzelner Zettel - das war eine Luecke.** Beim
 * Titelwechsel ueberlappen sich zwei Lieferungen fuer einige Sekunden: Der
 * Echo holt den naechsten Titel, waehrend der laufende noch kommt. Mit einem
 * einzigen Schluessel ueberschrieb die zweite die Marke der ersten, und war
 * sie zuerst fertig, raeumte sie ihn ab - die erste lief ungeschuetzt weiter.
 * Jetzt hat jede Lieferung ihren eigenen Eintrag in einer sortierten Menge,
 * der Wert ist ihr Verfallszeitpunkt. Ein neuer Schluesselname, damit sich
 * alte und neue Instanzen waehrend eines Deploys nicht in die Quere kommen.
 */
const LAEUFT_KEY = 'musik_ton_laeufer';
...
/** Wann ein Eintrag verfaellt, der jetzt geschrieben wird. */
const laeuftBis = () => Date.now() + LAEUFT_TTL_S * 1000;

/** Laeuft gerade eine Lieferung? Ein Fehler hier kostet nie die Wiedergabe. */
async function tonLaeuft(redis) {
  try {
    return Number(await redis.zcount(LAEUFT_KEY, Date.now(), '+inf')) > 0;
  } catch (err) {
...
async function laeuftMerken(redis) {
  ...
  try {
    await redis.zadd(LAEUFT_KEY, { score: laeuftBis(), member: marke });
    // Die Menge selbst verfaellt auch, falls nie wieder jemand liefert.
    await redis.expire(LAEUFT_KEY, LAEUFT_TTL_S);
  } catch { /* egal */ }
  return marke;
}

/**
 * Die Frist des eigenen Eintrags verlaengern, solange noch geliefert wird.
 *
 * `xx`: nur, wenn er noch da ist - ein Eintrag, den jemand schon abgeraeumt
 * hat, soll nicht wieder auferstehen.
 */
export async function laeuftAuffrischen(redis, marke) {
  try {
    await redis.zadd(LAEUFT_KEY, { xx: true }, { score: laeuftBis(), member: marke });
    await redis.expire(LAEUFT_KEY, LAEUFT_TTL_S);
  } catch { /* egal */ }
}

/**
 * Abgeraeumt wird nur der eigene Eintrag - die Lieferung nebenan behaelt
 * ihren. Dabei fallen auch Eintraege weg, deren Frist laengst um ist (eine
 * Function, die mitten im Schreiben starb).
 */
async function laeuftVergessen(redis, marke) {
  try {
    await redis.zrem(LAEUFT_KEY, marke);
    await redis.zremrangebyscore(LAEUFT_KEY, '-inf', Date.now());
  } catch { /* egal */ }
}
