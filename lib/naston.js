 ...
 */
export function frischeAdresse(url, stempel = Date.now()) {
  const roh = String(url ?? '');
  if (!/\/api\/skill\?ton=/.test(roh)) return url;
  return `${roh.replace(/&n=[^&]*/g, '')}&n=${stempel}`;
}
...
function durchleiten(koerper, res) {
  return new Promise((fertig) => {
    ...
    wache = frist ? setInterval(() => {
      if (Date.now() - zuletzt < frist) return;
      console.warn(`musik-box Ton steht still: seit ${Date.now() - zuletzt} ms kein Byte abgenommen`
        + `, ${lesbareMenge(bytes)} sind durch – die Lieferung wird beendet.`);
      strom.destroy();
      res.end();
      fertigEinmal();
    }, 1000) : null;
