 ...
 *
 * **"Faellt weg" hiess bisher nur "wirft nicht".** `JSON.parse("123")` ergibt
 * `123`, und eine Zahl ist wahr genug fuer `filter(Boolean)`. Im Dashboard
 * wurde daraus eine Zeile, in der jedes Feld `undefined` war - ein Eintrag,
 * den es nie gab, der aber aussah wie ein missglueckter Tonabruf. Ein Eintrag
 * ist ein Objekt und keine Liste; alles andere ist kein Eintrag.
 */
function alsEintrag(e) {
...
export async function verlaufLesen(redis) {
  try {
    ...
    return (roh || []).map(alsEintrag).filter(Boolean);
  } catch (err) {
...
async function tonLiefern(req, res, redis, rohToken, holeSid) {
  ...
  for (const erzwingen of [false, true]) {
    ...
    let bytes;
    try {
      bytes = await durchleiten(antwort.body, res);
    } finally {
