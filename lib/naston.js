 ...
 *
 * **Vier Megabyte, und die Zahl hat einen Grund.** Vercel begrenzt den Rumpf
 * einer Function auf 4,5 MB; nur eine wirklich *gestroemte* Antwort kommt
 * darueber hinaus, und ob eine Antwort gestroemt wird, entscheidet die
 * Laufzeit, nicht dieser Code. Gemeldet war genau das, was passiert, wenn sie
 * es nicht tut: `500 FUNCTION_INVOCATION_FAILED`. Vier Megabyte bleiben unter
 * der Grenze, egal wer recht hat - und sie passen nebenbei bequem in die Uhr
 * der Function (bei 10 Mbit/s Uplink gut drei Sekunden).
 *
 * Der Preis sind ein paar Bereichsabrufe mehr je Titel, und die kosten nichts:
 * Ein Abspieler holt sich den Rest ohnehin in Bereichen.
 */
const KAPPUNG_MB_VORGABE = 4;
...
async function tonLiefern(req, res, redis, rohToken, holeSid) {
  ...
  for (const erzwingen of [false, true]) {
    ...
    //
    // Gemeldet war `500 FUNCTION_INVOCATION_FAILED` - ein Absturz, kein Nein,
    // und lokal an einer echten HTTP-Verbindung nicht nachzustellen. Was sich
    // zwischen hier und dort unterscheidet, ist die Frage, ob die Laufzeit
    // eine Antwort wirklich stroemt oder sie erst einsammelt: Eingesammelt
    // gilt die Grenze von 4,5 MB, und eine Antwort darueber faellt genau so
    // um. Wer die Frage nicht beantworten kann, soll sie sich nicht stellen -
    // vier Megabyte passen in den Speicher wie in jede Grenze, und `res.end`
    // mit einem fertigen Puffer braucht vom Streamen nichts.
    //
    // Ohne Kappung (`MUSIK_TON_MAX_MB=0`) bleibt der Strom: Dort ist die Datei
    // beliebig gross, einsammeln keine Option, und wer die Kappung abschaltet,
    // weiss, dass seine Laufzeit stroemt.
    if (kappungBytes() > 0) {
      ...
      console.log(`musik-box Ton geliefert: ${adresseKurz(auftrag.pfad)} ${lesbareMenge(puffer.length)}`
    ...
    console.log(`musik-box Ton gestroemt: ${adresseKurz(auftrag.pfad)} ${lesbareMenge(bytes)}`
      + `${erzwingen ? ' (nach frischer Sitzung)' : ''}`);
    return undefined;
  }
