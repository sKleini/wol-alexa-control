...
export async function buildLocationSpeech(redis, person) {
  ...
  const sessionExpired = await relaySessionExpired(redis, person);
  ...
  if (sessionExpired) return `Ich kann den aktuellen Standort von ${person.name} nicht abrufen, weil die Anmeldung bei Samsung abgelaufen ist. Der letzte bekannte Standort war ${place}, zuletzt aktualisiert ${age}.`;
