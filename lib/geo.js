  ...
  return Promise.all(persons.map(async p => {
    ...
    return {
      ...
      location: {
        lat: loc.lat,
        lon: loc.lon,
        // Genauigkeit des Fix in Metern, so wie das Geraet sie gemeldet hat.
        // Steht bewusst neben lat/lon: Sie qualifiziert die Koordinaten, statt
        // eine eigene Auskunft zu sein - der Actions Hub zieht daraus den Kreis
        // um den Pin, damit ein 900-m-Fix aus einer Funkzelle nicht aussieht wie
        // ein 8-m-Fix vom GPS.
        //
        // `?? null` wie bei den Nachbarn: Ein Datensatz von vor dieser Aenderung
        // und jede Quelle ohne Unsicherheit haben das Feld nicht, und
        // "unbekannt" ist dort die richtige Antwort - nicht 0, das waere die
        // Behauptung eines perfekten Fix.
        //
        // Geschrieben wird `acc` seit jeher (api/location.js), nur diese Liste
        // liess es fallen: dieselbe Falle wie bei `vol`, und aus demselben
        // Grund unsichtbar - es kommt ja kein Fehler, es fehlt nur etwas.
        acc: loc.acc ?? null,
        ...
        // Wie alt der **Zustand** ist - im Unterschied zu `ageSec`, das die
        // **Position** meint. Die beiden laufen auseinander, seit Mylo
        // Statusmeldungen ohne Position schickt: Danach ist der Klingelmodus
        // Sekunden alt, die Position aber weiter zwei Stunden.
        //
        // Die Actions-App laesst Klingelmodus und Taschenlampe nach 30 bzw. 6
        // Minuten verfallen. Rechnete sie das weiter mit `ageSec`, verwuerfe
        // sie soeben gemeldete Angaben als veraltet - ausgerechnet bei einem
        // Handy, das seit Stunden keinen Fix mehr hinbekommt, und das ist die
        // Lage, in der man am ehesten wissen will, ob es klingeln wuerde.
        statusAgeSec: Number.isFinite(loc.receivedAt)
          ? Math.max(0, nowSec - loc.receivedAt)
          : null,
        batt: loc.batt,
        ...
        // richtige Auskunft. Wie alt sie ist, sagt `statusAgeSec` gleich mit.
        ring: loc.ring ?? null,
        dnd: loc.dnd ?? null,
        // Laeuft "Nicht stoeren" gerade? Nicht zu verwechseln mit `dnd` - das
        // sagt nur, ob Mylo den Klingelmodus aendern DARF. Ist zen true,
        // erklaert das ein gemeldetes ring=silent bei einem Handy, dessen
        // Tonprofil "Ton" anzeigt.
        zen: loc.zen ?? null,
        torch: loc.torch ?? null,
        // Die Lautstaerken der Tonkanaele, `media=70,ring=100`. Steht in keiner
        // Zeile der Liste - fuenf Prozentzahlen waeren dort Rauschen -, sondern
        // belegt den Regler-Dialog vor. `?? null` wie bei den Nachbarn: Wo
        // nichts gemeldet wurde, sperrt die App die Regler, statt einen Wert
        // zu erfinden.
        vol: loc.vol ?? null,
        chg: loc.chg ?? null,
        // Verbindungsart und WLAN-Name. Der WLAN-Name ist der von GPS
        // unabhaengige Hinweis darauf, dass jemand zu Hause ist - drinnen oft
        // verlaesslicher als der Fix.
        net: loc.net ?? null,
        ssid: loc.ssid ?? null,
        // Warum kommt nichts Neues? Die beiden beantworten es, statt eine
        // stehengebliebene Position unkommentiert altern zu lassen.
        air: loc.air ?? null,
        gps: loc.gps ?? null,
        // Die Freigabe fuer die sichtbare Meldung. Muss hier stehen, sonst
        // faellt sie auf dem Rueckweg wieder heraus - genau die Falle, in die
        // vol und acc schon einmal geraten sind: Es kommt kein Fehler, es
        // fehlt nur etwas.
        ovl: loc.ovl ?? null,
        // Die Fassung der Mylo-App auf dem Geraet, damit der Actions Hub sie
        // im Menue des langen Tipps zeigen kann. Dieselbe Pflicht wie die
        // Zeile darueber: Was hier fehlt, existiert fuer die App nicht - und
        // zwar lautlos.
        ver: loc.ver ?? null,
        // Der wirksame Sendetakt in Minuten. Dieselbe Pflicht und dieselbe
        // stille Falle wie zwei Zeilen darueber - ohne diese Zeile kaeme die
        // Zahl zwar an, erreichte den Hub aber nie.
        takt: loc.takt ?? null,
        // Und die Zahl der Zaeune auf dem Geraet - dieselbe Pflicht wie die
        // Zeile darueber, sonst kaeme sie an und erreichte den Hub nie.
        zonen: loc.zonen ?? null,
        // Die Bewegungslage des Geraets - dieselbe Pflicht und dieselbe stille
        // Falle wie die Zeilen darueber: Ohne sie kaeme der Wert an, wuerde
        // gespeichert und erreichte den Hub trotzdem nie.
        mov: loc.mov ?? null,
        // Zeitpunkte, keine Flags: "war ab 21:14 aus" bleibt wahr, bis das
        // Gegenereignis kommt.
        bootedAt: loc.bootedAt ?? null,
        offAt: loc.offAt ?? null,
        pausedAt: loc.pausedAt ?? null,
      },
    };
  }));
}
