...
// sie und sind damit **begrenzt haltbar** - der Import sagt das auch. Die
// dauerhafte Loesung ist, die Adresse erst beim Abspielen zu bauen; dieser
// Weg legt die Bausteine dafuer (siehe `streamUrl`).
 ...
 *  und Alexa gibt ihm acht Sekunden fuer die ganze Antwort. */
const SID_MS = 4000;
...
export async function importiereFritzOrdner(freigabe, maxTitel) {
  ...
  bericht.push(`Probe auf den ersten Titel: ${probe.kurz}`);
  if (!probe.ok) {
    return {
  ...
  const hinweise = [
    ...
    'Die Adressen tragen eine Sitzungsnummer der FRITZ!Box und sind deshalb nur begrenzt haltbar – meist etwa zwanzig Minuten nach dem letzten Zugriff. Laeuft sie ab, bleibt der Echo stumm; dann den Ordner erneut importieren.',
  ];
...
async function pruefeStream(url, rest) {
  ...
  return { ok, range, kurz: `HTTP ${antwort.status}, ${typ || 'ohne Inhaltstyp'}${range ? ', Bereiche' : ''}` };
}
 ...
 */
export async function frischeSid(freigabe) {
  const bericht = [];
  const frist = Date.now() + SID_MS;
  const rest = () => Math.max(400, Math.min(SCHRITT_MS, frist - Date.now()));
