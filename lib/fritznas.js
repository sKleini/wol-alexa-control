 ...
 * "ohne Antwort" stand.
 *
 * @returns {Promise<{gilt: true|false|null, kurz: string}>} gilt / gilt nicht /
 *   nicht zu erfahren, dazu in einem Wort, was die Box gesagt hat
 */
export async function sitzungGilt(freigabe, sid, budgetMs = PRUEF_MS) {
  if (!freigabe?.herkunft || !sid) return { gilt: false, kurz: 'keine Nummer' };

  const grund = await zielErlaubt(new URL(freigabe.herkunft));
  if (grund) return { gilt: null, kurz: grund };
  ...
  if (!antwort.daten) return { gilt: null, kurz: antwort.kurz };
  return { gilt: Boolean(antwort.daten.root || antwort.daten.rights), kurz: antwort.kurz };
}
...
// Bereichsabruf ueber ein einziges Byte, genau die Adresse, die gleich in der
// Direktive steht. Er dauert, was er dauert, wird nach ein paar Sekunden
// abgebrochen - und lange bevor er fertig ist, dreht die Platte. Der Echo
// findet sie dann wach vor, so wie es sonst erst der zweite Versuch tat.
//
...
/**
 * Die Datei antippen: ein Byte, mehr nicht.
 *
 * Liefert `{ ok, endgueltig, ms, kurz }`. `endgueltig: false` heisst "die Box
 * hat nichts gesagt" - keine Antwort in der Frist, nicht erreichbar, Ziel
 * nicht erlaubt. Der Aufrufer spielt dann trotzdem los: Der Weckruf hat seine
...
export async function weckeStream(url, budgetMs = WECK_MS) {
  ...
  let ziel;
  try { ziel = new URL(url); } catch { return { ok: false, endgueltig: false, ms: 0, kurz: 'keine Adresse' }; }
  const grund = await zielErlaubt(ziel);
  if (grund) return { ok: false, endgueltig: false, ms: seit(), kurz: grund };
  ...
  try {
    antwort = await fetch(ziel, {
      method: 'GET',
      redirect: 'manual',
      headers: { Range: 'bytes=0-0', 'User-Agent': 'musik-box-weckruf/1.0' },
  ...
  // Der Inhalt interessiert nicht, nur der Kopf. Was noch kommt, wird
  // weggeworfen: Der Weckruf soll die Leitung nicht laenger halten als noetig.
  try { await antwort.body?.cancel(); } catch { /* egal */ }

  // Was die Box gesagt hat, ist endgueltig - sie hat ja geantwortet.
  const { ok, typ } = weckUrteil(antwort.status, antwort.headers.get('content-type'));
  return { ok, endgueltig: true, ms: seit(), kurz: `HTTP ${antwort.status}, ${typ || 'ohne Inhaltstyp'}` };
}
