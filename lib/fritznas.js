...
export async function importiereFritzOrdner(freigabe, maxTitel) {
  ...
  if (probe.zuLang) {
    bericht.push(
      'Hinweis: Diese Datei spielt laenger als eine Stunde. Der Ton laeuft in '
      + 'Stuecken durch die App (siehe lib/naston.js), das haelt sie aus - in '
      + 'Kapitel geteilt setzt "Weiterhoeren" aber an einer sinnvollen Stelle '
      + 'ein statt mitten in einer Stunde.',
    );
  }
  if (!probe.ok) {
    return {
  ...
  const hinweise = [
    ...
    'Die Adressen zeigen auf diese App, nicht auf die FRITZ!Box: Eine Ordner-Freigabe gibt ihre Dateien nur mit einer Sitzungsnummer heraus, und die gilt nur fuer die Adresse, die sie geholt hat – fuer den Echo waere sie wertlos. Die App holt den Ton deshalb selbst und reicht ihn durch. Mit "Save Playlist" speichern, damit der Ordner-Link bei der Playlist bleibt.',
  ];
