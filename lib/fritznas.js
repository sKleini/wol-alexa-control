 ...
 *
 * Gibt bei jedem Zweifel `false` zurueck - nicht erreichbar, kein JSON, keine
 * Rechte. Der Aufrufer meldet sich dann neu an; eine falsche Zusage waere
 * teurer als eine ueberfluessige Anmeldung.
 */
export async function sitzungGilt(freigabe, sid, budgetMs = PRUEF_MS) {
  ...
  return Boolean(antwort.daten && (antwort.daten.root || antwort.daten.rights));
}
