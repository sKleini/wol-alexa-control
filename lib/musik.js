...
async function pruefeUrl(start) {
  const ergebnis = { url: start, status: null, contentType: null, range: false, fehler: null, warnung: null };
  let url;
  try { url = new URL(start); } catch { ergebnis.fehler = 'keine gueltige URL'; return ergebnis; }

  // Ein abweichender Port ist kein Fehler des Abrufs - der Browser laedt die
  // Datei tadellos, und genau das macht ihn so schwer zu finden: Der
  // Alexa-AudioPlayer laedt ausschliesslich ueber 443 und meldet nichts,
  // wenn es woanders steht. Die FRITZ!Box liefert ihren Fernzugriff ab Werk
  // auf 456, also trifft das den haeufigsten Fall dieser Playlists.
  if (url.port && url.port !== '443') {
    ergebnis.warnung = `Port ${url.port} – Alexa laedt nur ueber Port 443`;
  }
...
/**
 * Was an den gefundenen Adressen dem Echo spaeter im Weg steht.
 *
 * Der Alexa-AudioPlayer laedt **nur ueber Port 443**. Genau daran scheitert
 * der Weg ueber die FRITZ!Box im Auslieferungszustand: ihr HTTPS-Fernzugriff
 * liegt auf Port 456, der Link funktioniert im Browser tadellos, und Alexa
 * bleibt stumm. Das hier ist die einzige Stelle, an der das auffallen kann,
 * bevor jemand eine halbe Stunde lang den Skill verdaechtigt.
 */
export function importHinweise(urls) {
  const hinweise = [];
  const ports = new Set();
  for (const u of urls) {
    try {
      const url = new URL(u);
      if (url.port && url.port !== '443') ports.add(url.port);
    } catch { /* schon gefiltert */ }
  }
  if (ports.size) {
    hinweise.push(`Die Adressen liegen auf Port ${[...ports].join(', ')}. Alexa laedt Audiodateien ausschliesslich ueber Port 443 – in der FRITZ!Box unter "Internet › Freigaben › FRITZ!Box-Dienste" den HTTPS-Port auf 443 stellen und den Freigabe-Link neu kopieren.`);
  }
  return hinweise;
}
...
export async function handleImport(req, res) {
  ...
  if (/^audio\//.test(inhaltstyp)) {
    return res.status(200).json({
      ...
      hinweise: ['Der Link zeigt auf eine einzelne Audiodatei, nicht auf einen Ordner.', ...importHinweise([url.href])],
    });
  }
  ...
  const hinweise = importHinweise(httpsNur);
  ...
  if (!httpsNur.length) {
    ...
    return res.status(200).json({ url: url.href, titel: [], hinweise, error: fehler });
  }
