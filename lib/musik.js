 ...
 */
async function weckeErstenTitel(playlist, direktive, rest) {
  if (playlist?.quelle?.typ !== 'fritz' || !direktive) return true;
  const frei = rest() - SID_ANTWORT_RESERVE_MS;
  if (frei < WECKRUF_MINDESTBUDGET_MS) {
    console.warn(`musik-box Weckruf ausgelassen, nur noch ${rest()} ms Budget`);
    return true;
  }
  const antwort = await weckeStream(direktive.audioItem.stream.url, Math.min(WECKRUF_MS, frei));
  // **Diese Zeile ist die Gegenprobe zur Vermutung.** Steht hier "HTTP 206,
  // audio/mpeg nach 140 ms" und der Echo bleibt trotzdem stumm, lag es nicht
  // an der Platte - dann ist die Datei zur Antwortzeit abrufbar gewesen, und
  // die naechste Suche faengt beim Geraet an statt wieder bei der Box.
  console.log(`musik-box Datei angetippt: ${antwort.kurz}`
    + ` nach ${antwort.ms} ms, ${budgetText(rest())}`);
  return !(antwort.endgueltig && !antwort.ok);
}
...
async function fritzSid(link, redis, erzwingen = false, rest = () => Infinity) {
  ...
  if (eigene && rest() >= SID_PRUEF_MINDESTBUDGET_MS) {
    ...
    const wort = gilt === null ? 'ohne Antwort' : gilt ? 'gilt noch' : 'ist tot';
    ...
    if (gilt === null && passt) return { sid: eigene, verlaesslich: true, erneuert: false };
  } else if (passt) {
...
/**
 * Die Sitzung warm halten - angestossen von aussen, ohne Alexa.
 *
 * **Warum das nicht der Skill selbst tun kann.** Der stumme erste Versuch ist
 * immer der mit dem Login. Ein Login kostet zwei Abrufe bei einer langsamen
 * Box (gemessen: 1951 ms), und er liegt ausgerechnet in dem Request, der
 * Alexas Fenster teilen muss - der zweite Versuch, der die Nummer gemerkt
 * vorfindet, ist in einem Drittel der Zeit fertig und spielt. Der Skill kann
 * diesen Login nur verschieben, nicht vermeiden: Vor dem ersten Ton muss die
 * Nummer da sein, und beim Ein-Satz-Aufruf ("Alexa, spiele X bei meiner
 * Plattenkiste") gibt es keinen LaunchRequest, in dessen Ruhe er fiele.
 *
 * Wer von aussen alle paar Minuten anklopft, nimmt ihn ganz heraus: Die Box
 * verlaengert die Sitzung bei jedem Zugriff, und wer sie nie ablaufen laesst,
 * meldet sich nie wieder an. Der erste Versuch sieht dann aus wie der zweite.
 *
 * **Angemeldet wird trotzdem, wenn es sein muss.** `fritzSid` geht dieselben
 * drei Stufen wie im Skill: gemerkt, nachgefragt, und erst bei einer wirklich
 * toten Nummer die Anmeldung. Erzwungen wird nur die Nachfrage - ohne sie
 * liefe dieser Aufruf innerhalb der Frist ins Leere und fasste die Box gar
 * nicht an, also genau das Gegenteil seines Zwecks.
 *
 * **Nur bei genau einer Freigabe** - derselbe Grund wie bei
 * `sitzungVorwaermen`: Jede Anmeldung beendet alle Sitzungen der Box, und die
 * falsche Wahl naehme der richtigen gerade die ihre.
 *
 * **Ohne Frist.** Hier wartet niemand; die acht Sekunden sind Alexas, nicht
 * die eines Cron-Laufs.
 */
export async function handleWarm(req, res, redis) {
  // POST, weil der Aufruf etwas tut: Er fasst die Box an und kann im
  // Notfall anmelden. Ein GET, den ein Browser-Prefetch beilaeufig ausloest,
  // waere dafuer die falsche Tuer.
  ...
  if (links.length !== 1) {
    const grund = links.length === 0
      ? 'keine FRITZ!NAS-Playlist'
      : `${links.length} Ordner-Freigaben – welche gemeint ist, waere geraten`;
    console.log(`musik-box Sitzung warm halten ausgelassen: ${grund}`);
    return res.status(200).json({ ok: true, warm: false, grund });
  }

  const begonnen = Date.now();
  const { sid, verlaesslich, erneuert } = await fritzSid(links[0], redis, true);
  const ms = Date.now() - begonnen;
  console.log(`musik-box Sitzung warm gehalten: ${verlaesslich ? 'ja' : 'nein'}`
    + `${erneuert ? ', neu angemeldet' : ''} nach ${ms} ms`);
  return res.status(200).json({
    ok: verlaesslich,
    warm: verlaesslich,
    erneuert,
    ms,
    // **Nie die ganze Nummer.** Sie ist der Schluessel zur Freigabe, und diese
    // Antwort landet in /var/log auf dem VPS. Die letzten vier Stellen
    // genuegen, um sie mit dem Skill-Log zu vergleichen - so wie adresseKurz
    // es dort auch haelt.
    ...
    if (typ === 'IntentRequest') {
      ...
      switch (intent.name) {
        ...
        case 'AMAZON.StartOverIntent':
          return await vonVorn(body, res, playlists, redis, direktiven, rest);
...
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity, fritzOk = true) {
  ...
  if (!await weckeErstenTitel(playlist, start[0], rest)) {
 ...
 */
async function losOderAbsage(body, res, playlist, start, direktiven = [], rest = () => Infinity) {
  if (await weckeErstenTitel(playlist, start, rest)) return still(res, [start], ausSprache(body));
...
async function weiter(body, res, playlists, direktiven = [], stumm = false, redis = null, rest = () => Infinity) {
  ...
  if (lage) {
    ...
    return await losOderAbsage(body, res, lage.playlist, start, direktiven, rest);
  }
  ...
  if (juengste) {
    ...
    return await losOderAbsage(body, res, juengste.playlist, start, direktiven, rest);
  }
...
async function vonVorn(body, res, playlists, redis, direktiven = [], rest = () => Infinity) {
  const lage = laufendes(body, playlists);
