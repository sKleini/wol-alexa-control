 ...
 *
 * **Mit einer frisch geholten Sitzungsnummer wird derselbe Titel wiederholt.**
 * `fritzErneuert` heisst: Die Nummer, mit der dieser Titel geladen wurde, war
 * tot, und eine neue liegt vor. Dann ist nicht der Titel kaputt, sondern nur
 * die Adresse war es - und ihn zu ueberspringen hiesse, den Hoerer fuer einen
 * Fehler der Box zu bestrafen. Er faengt dort wieder an, wo er abbrach
 * (abzueglich des ueblichen Vorlaufs), als waere nichts gewesen.
 *
 * **Und er wiederholt sich genau einmal.** Scheitert er auch mit der neuen
 * Nummer, sagt die Nachfrage im naechsten Request "gilt noch", es wird nicht
 * erneut angemeldet, `fritzErneuert` ist falsch - und es geht weiter wie
 * bisher, mit dem naechsten Titel. Eine Schleife kann daraus nicht werden.
 */
function nachFehler(body, res, playlists, fritzErneuert = false) {
  ...
  console.warn('Alexa konnte nicht abspielen:', fehler.type, fehler.message,
    ...
    wer ? `Titel: ${wer}` : '',
    fritzErneuert ? '- neue Sitzungsnummer, zweiter Versuch' : '- weiter mit dem naechsten Titel');
  if (!lage) return still(res);
  ...
  if (fritzErneuert && lage.token.position < lage.playlist.titel.length) {
    return still(res, [playDirektive(lage.playlist, lage.token.position, lage.token.runde, {
      seed: lage.token.seed, offset: einstieg(lage.offset),
    })]);
  }
  const ziel = schritt(lage.playlist, lage.token, +1);
