 ...
 *
 * **`wiederholen` fehlt im Body: bisheriger Wert, sonst an.** Dasselbe Muster
 * wie `home` bei den Zonen in api/manage.js, und aus demselben Grund: Ein
 * Aufrufer, der das Feld nicht kennt (ein Skript, eine aeltere Oberflaeche),
 * soll eine Titelliste korrigieren koennen, ohne nebenbei die Wiederholung
 * umzulegen. Die Kehrseite ist, dass ein Abwaehlen ausdruecklich als
 * `wiederholen: false` ankommen muss - das Dashboard schickt das Feld deshalb
 * immer mit. Bei einer neuen Playlist ist "an" die Vorgabe, weil es das
 * Verhalten vor dieser Einstellung war.
 *
 * @param {object} body
 * @param {object|null} [bisher] die gespeicherte Playlist gleichen Namens
 * @returns {{ playlist: {name: string, titel: {url: string, name: string}[], wiederholen: boolean} } | { fehler: string }}
 */
export function validierePlaylist(body, bisher = null) {
  ...
  const wiederholen = body && 'wiederholen' in body
    ? !!body.wiederholen
    : wiederholtSich(bisher);
  return { playlist: { name, titel, wiederholen } };
}

/**
 * Wiederholt sich diese Playlist?
 *
 * **Ein fehlendes Feld heisst "ja".** Die Playlists, die vor dieser Einstellung
 * angelegt wurden, tragen es nicht, und sie liefen endlos - genau das muessen
 * sie weiter tun, ohne dass jemand sie anfasst. `null` (gar keine Playlist)
 * ergibt ebenfalls "ja", denn das ist der Fall "neu angelegt".
 *
 * Rein und exportiert, damit die Vorgabe an einer Stelle steht statt an fuenf.
 */
export function wiederholtSich(playlist) {
  return playlist?.wiederholen !== false;
}
