 ...
 *
 * **Angemeldet wird nur bei genau einer Freigabe.** Eine Anmeldung beendet
 * alle Sitzungen der Box. Bei mehreren Ordner-Playlists wuerde ein Takt, der
 * anmeldet, reihum immer die anderen umbringen - erst A, fuenf Minuten
 * spaeter B und damit A wieder, und so fort. Dort haelt dieser Aufruf nur am
 * Leben, was lebt; eine tote Nummer bleibt tot, bis jemand diese Playlist
 * wirklich spielt, und der Fehlerweg holt dann eine neue.
 *
...
export async function handleWarm(req, res, redis) {
  ...
  // Eine einzige Freigabe darf sich anmelden - es gibt keine andere, der das
  // schadete. Bei mehreren wird nur nachgefragt, siehe oben.
  const darfAnmelden = freigaben.size === 1;
  const frist = Date.now() + WARM_GESAMT_MS;
  const ergebnisse = [];
  for (const [link, ordner] of freigaben) {
    ...
    const begonnen = Date.now();
    const { sid, verlaesslich, angemeldet } = await fritzSid(
      link, redis, true, rest, () => Infinity, ordner,
      darfAnmelden ? '' : 'mehrere Freigaben – eine Anmeldung wuerde die anderen beenden',
    );
    ergebnisse.push({
      link: freigabeKurz(link),
      warm: verlaesslich,
      angemeldet,
      ms: Date.now() - begonnen,
      sid: sid ? `…${sid.slice(-4)}` : null,
    });
  }
  ...
  return res.status(200).json({ ok: warm > 0, warm, gesamt: freigaben.size, freigaben: ergebnisse });
}
