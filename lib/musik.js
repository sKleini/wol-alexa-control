...
import { tonUrl, alsTonUrl, eigeneBasis, tonSchluessel, monatsStand, budgetBytes, lesbareMenge } from './naston.js'
...
/**
 * Was eine Anmeldung ohne Gegenprobe kostet - und die Schwelle, ab der sie
 * vor dem ersten Ton unterbleibt.
 *
...
async function mitFrischerSid(playlist, redis, erzwingen = false, rest = () => Infinity, restEil = () => Infinity) {
  ...
  const { sid, verlaesslich, erneuert, ungeprueft, angemeldet } = await fritzSid(
    playlist.quelle.link, redis, erzwingen, rest, restEil, playlist.quelle.ordner || null,
  );
 ...
 */
export async function fritzSid(link, redis, erzwingen = false, rest = () => Infinity, restEil = () => Infinity, ordner = null) {
  ...
  // ausfuehrt.
  if (eigene && restEil() < LOGIN_ERWARTET_MS) {
    console.warn(`musik-box Anmeldung ausgelassen, nur noch ${restEil()} ms Eilziel`
      + ' – es wird mit der alten Nummer gestartet, ein Fehlschlag kommt als PlaybackFailed zurueck');
    return { sid: eigene, verlaesslich: false, erneuert: false, ungeprueft: true, angemeldet: false };
  }
