...
/** Ein zweiter Weckruf lohnt nur, wenn er auch eine Antwort bekommen kann. */
const WECKRUF_ZWEITER_MINDEST_MS = 600;
 ...
 */
async function weckeErstenTitel(playlist, direktive, rest, nachLogin = false) {
  ...
  const abstand = Math.max(0, Math.min(
    WECKRUF_ABSTAND_MS(),
    rest() - SID_ANTWORT_RESERVE_MS - WECKRUF_ZWEITER_MINDEST_MS,
  ));
  if (abstand <= 0) {
 ...
 * faengt woanders an.
 */
async function einWeckruf(url, rest, was) {
...
async function fritzAufgefrischt(playlists, body, redis, rest = () => Infinity) {
  if (!playlists.some(p => p.quelle?.typ === 'fritz')) return { playlists, fritzOk: true, fritzErneuert: false, fritzUngeprueft: false, fritzAngemeldet: false };

  const gemeint = gemeintePlaylist(playlists, body);
  if (gemeint?.quelle?.typ !== 'fritz') return { playlists, fritzOk: true, fritzErneuert: false, fritzUngeprueft: false, fritzAngemeldet: false };
  ...
  const { playlist: frisch, verlaesslich, erneuert, ungeprueft, angemeldet } = await mitFrischerSid(gemeint, redis, erzwingen, rest);
  return {
    playlists: playlists.map(p => (p === gemeint ? frisch : p)),
    fritzOk: verlaesslich,
    fritzErneuert: erneuert,
    fritzUngeprueft: ungeprueft,
    fritzAngemeldet: angemeldet,
  };
}
...
async function mitFrischerSid(playlist, redis, erzwingen = false, rest = () => Infinity) {
  ...
  const { sid, verlaesslich, erneuert, ungeprueft, angemeldet } = await fritzSid(playlist.quelle.link, redis, erzwingen, rest);
...
export async function handleSkill(body, res, redis) {
  ...
  const { fritzOk, fritzErneuert, fritzUngeprueft, fritzAngemeldet } = aufgefrischt;
  ...
  try {
    ...
    if (typ === 'IntentRequest') {
      ...
      switch (intent.name) {
        ...
        case 'SuchePlaylistIntent':
          return await handlePlay(intent, res, playlists, direktiven, redis, rest, fritzOk, fritzUngeprueft, fritzAngemeldet);
        ...
        case 'AMAZON.ResumeIntent':
          return await weiter(body, res, playlists, direktiven, false, redis, rest, fritzAngemeldet);
        case 'AMAZON.NextIntent':
          return springe(body, res, playlists, +1);
        case 'AMAZON.PreviousIntent':
          return springe(body, res, playlists, -1);
        case 'AMAZON.StartOverIntent':
          return await vonVorn(body, res, playlists, redis, direktiven, rest, fritzAngemeldet);
    ...
    if (typ === 'PlaybackController.PlayCommandIssued') return await weiter(body, res, playlists, [], true, redis, rest, fritzAngemeldet);
 ...
 */
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity, fritzOk = true, fritzUngeprueft = false, fritzAngemeldet = false) {
  if (playlists.length === 0) {
    return speak(res, 'Es ist noch keine Playlist angelegt. Bitte lege im Dashboard eine an.', true);
  }
  ...
  if (!await weckeErstenTitel(playlist, start[0], rest, fritzAngemeldet)) {
 ...
 */
async function losOderAbsage(body, res, playlist, start, direktiven = [], rest = () => Infinity, nachLogin = false) {
  if (await weckeErstenTitel(playlist, start, rest, nachLogin)) return still(res, [start], ausSprache(body));
 ...
 */
async function weiter(body, res, playlists, direktiven = [], stumm = false, redis = null, rest = () => Infinity, nachLogin = false) {
  const lage = laufendes(body, playlists);
  if (lage) {
    ...
    return await losOderAbsage(body, res, lage.playlist, start, direktiven, rest, nachLogin);
  }
  ...
  if (juengste) {
    ...
    return await losOderAbsage(body, res, juengste.playlist, start, direktiven, rest, nachLogin);
  }
...
async function vonVorn(body, res, playlists, redis, direktiven = [], rest = () => Infinity, nachLogin = false) {
  const lage = laufendes(body, playlists);
