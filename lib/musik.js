...
async function fritzAufgefrischt(playlists, body, redis) {
  ...
  const frisch = await mitFrischerSid(gemeint, redis, erzwingen);
 ...
 */
async function mitFrischerSid(playlist, redis, erzwingen = false) {
  if (playlist?.quelle?.typ !== 'fritz') return playlist;
  const sid = await fritzSid(playlist.quelle.link, redis, erzwingen);
...
async function fritzSid(link, redis, erzwingen = false) {
  ...
  const gemerkt = await fritzSidGemerkt(redis);
 ...
 */
async function fritzSidGemerkt(redis) {
  try {
    const bestand = await redis.get(FRITZ_SID_KEY);
