...
async function fritzAufgefrischt(playlists, body, redis, rest = () => Infinity, restEil = () => Infinity) {
  ...
  const { playlist: frisch, verlaesslich, erneuert, ungeprueft, angemeldet } = await mitFrischerSid(
    gemeint, redis, erzwingen, rest, eile, faengtAn(body) && startOhneAnmeldung(),
  );
  return {
 ...
 */
async function mitFrischerSid(playlist, redis, erzwingen = false, rest = () => Infinity, restEil = () => Infinity, ohneAnmeldung = false) {
 ...
 */
async function fritzSid(link, redis, erzwingen = false, rest = () => Infinity, restEil = () => Infinity, ordner = null, ohneAnmeldung = false) {
  ...
  // ausfuehrt.
  if (ohneAnmeldung) {
    console.warn(`musik-box Anmeldung vor dem ersten Ton ausgelassen`
      + `${eigene ? '' : ' (und es gibt keine gemerkte Nummer)'}`
      + ' – gestartet wird mit dem, was da ist; ein Fehlschlag kommt als PlaybackFailed zurueck');
    return { sid: eigene, verlaesslich: false, erneuert: false, ungeprueft: true, angemeldet: false };
  }
...
async function nachFehler(body, res, playlists, fritzErneuert = false, rest = () => Infinity) {
  ...
  // Beim ersten Titel einer Straehne den Befund dazuschreiben, wenn der
  // Fehlertyp ihn nahelegt - siehe `ipGebundenMelden`.
  if (straehneBisher === 0 && lage.token.versuch === 0) ipGebundenMelden(fehler.type, lage.playlist);
...
/**
 * Der Befund hinter `MEDIA_ERROR_INTERNAL_SERVER_ERROR` - eine Zeile, einmal
 * je Pechstraehne.
 *
 * **Die FRITZ!Box bindet ihre Sitzungen an die IP-Adresse, die sie geholt
 * hat.** Sie schreibt es in ihre eigene Ereignisanzeige:
 *
 ...
 *   beendet. [12 Meldungen seit 14:59:19]
 *
 * Zwei Adressen tauchen dort auf: die von Vercel (AWS Frankfurt), also dieser
 * Skill, und die **externe IPv4 der Box selbst** - so sieht sie den Echo, der
 * den MyFRITZ!-Namen aufloest und per NAT-Loopback von aussen hereinkommt.
 * Zwei verschiedene Adressen, und damit zwei verschiedene Sitzungen: Eine
 * Nummer, die der Skill holt, ist fuer den Echo **ungueltig**, ganz gleich wie
 * frisch sie ist.
 *
 * **Damit ist erklaert, was diese Datei seit dem ersten Log umkreist.** Der
 * Skill prueft die Nummer und bekommt "gilt noch"; er ruft die Datei ab und
 * bekommt HTTP 206 mit Tondaten - beides von seiner IP aus, fuer die die
 * Sitzung ja gilt. Der Echo bekommt mit derselben Adresse einen Fehler, und
 * die Box beendet obendrein alle Sitzungen seiner IP. Gemessen ueber alle
 * Logs eines Tages:
 *
 *   MEDIA_ERROR_SERVICE_UNAVAILABLE   4x → 4x PlaybackStarted danach
 *   MEDIA_ERROR_INTERNAL_SERVER_ERROR 21x → 0x
 *
 * Der erste Fehler ist ein Verbindungsproblem und heilt beim zweiten Anlauf.
 * Der zweite ist die abgelehnte Sitzungskennung, und dagegen hilft nichts,
 * was dieser Skill tun kann: keine frische Nummer, kein Weckruf, kein
 * Wiederholen. Eine Ordner-Freigabe traegt ihre Sitzung in der Adresse, und
 * diese Sitzung gehoert nicht dem Geraet, das sie benutzen soll.
 *
 * Was hilft, liegt ausserhalb: eine **Datei**-Freigabe, deren Link ohne
 * Sitzung auskommt. Die Zeile hier sagt das dem naechsten Leser des Logs,
 * statt ihn dieselbe Runde noch einmal drehen zu lassen.
 */
function ipGebundenMelden(fehlerTyp, playlist) {
  if (playlist?.quelle?.typ !== 'fritz') return;
  if (fehlerTyp !== 'MEDIA_ERROR_INTERNAL_SERVER_ERROR') return;
  console.warn('musik-box Dieser Fehler ist die abgelehnte Sitzungskennung:'
    + ' Die Box bindet eine Sitzung an die IP, die sie geholt hat (hier Vercel),'
    + ' und fuer den Echo ist dieselbe Nummer damit ungueltig – nachzulesen in der'
    + ' Ereignisanzeige der FRITZ!Box. Eine frische Nummer aendert daran nichts;'
    + ' zuverlaessig spielt eine Playlist aus Datei-Freigaben, die ohne Sitzung auskommen.');
}
