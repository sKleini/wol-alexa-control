...
/**
 * Eine Null ist keine Stelle, sondern eine fehlende Auskunft.
 *
 * **Die Regel, die dem Hoerbuch gefehlt hat.** Innerhalb desselben Titels
 * wird der Stand nicht ueberschrieben, *wenn der neue Wert null ist*: Ein
 * Titelanfang bei 0:00 sagt nichts darueber, dass die zehnte Minute nicht mehr
 * gilt. Genau daran ist das sekundengenaue Fortsetzen gescheitert - ein
 ...
 * selbst festhielt.
 *
 * **Keine Monotonie, nur diese Null.** Jeder andere Wert wird geschrieben,
 * auch ein kleinerer: Der Vorlauf knabbert bei jedem Weiterhoeren fuenf
 * Sekunden ab, und wer nach einem Sprung zurueck bei 0:30 stoppt, ist dort und
 * nicht bei 10:00. Eine Sperre "nie kleiner" waere eine Wette darauf, dass
 * Position, Runde und Mischung je Strom nur einmal vorkommen - sie kommen nach
 * jedem Wiedereinstieg wieder.
 */
export function standNichtZurueck(stand, token, offset) {
  if (!stand || offset > 0) return false;
  if (!selberTitel(stand, token)) return false;
  return (Number(stand.offset) || 0) > 0;
}
...
async function standMerken(body, playlists, redis, rest = () => Infinity, { offset = null, quelle = 'ereignis' } = {}) {
  ...
  // mit einer Null zudeckt. Er laeuft hinter der Antwort (siehe `nachspiel`),
  // kostet also keine Millisekunde, auf die jemand wartet.
  ...
  if (standNichtZurueck(stand, lage.token, stelle)) {
    return `Null verworfen, ${stand.offset} ms stehen schon`;
  }
  ...
  return `${stelle} ms gemerkt (${quelle})`;
}
 ...
 *
 * Geschrieben wird im Nachspiel, hinter der Antwort: Wer "Stopp" sagt, will
 * Stille und keine Geduld.
 */
function haltUndMerken(body, res, playlists, redis, schluss, notiz) {
  notiz.stelle = body.context?.AudioPlayer?.offsetInMilliseconds || 0;
  notiz.woher = 'geraet';
  notiz.nachspiel = (frist) => standMerken(body, playlists, redis, frist, {
    offset: notiz.stelle,
    quelle: 'wort',
  });
...
/**
 * Die Frist fuer alles, was erst nach der Antwort laeuft.
 *
 * **Warum das Nachspiel eine eigene bekommt.** `rest()` ist dort negativ -
 * die Antwort ist ja heraus -, und `mitFrist` kehrt bei einem Budget unter
 * null sofort mit `kein Zeitbudget mehr` um. Wer den gemerkten Stand hinter
 * die Antwort schiebt und ihm `rest` mitgibt, hat ihn abgeschafft, nicht
 * verlegt. Lang genug fuer zwei Redis-Abrufe, kurz genug, dass eine haengende
 * Datenbank die Function nicht bis `maxDuration` wachhaelt.
 */
const NACHSPIEL_FRIST_MS = 2000;
...
export async function handleSkill(body, res, redis) {
   ...
   *
   * **`nachspiel` ist das Wichtigste daran.** Der gemerkte Stand stand bisher
   * unter demselben Budget wie die Antwort, und `standSchreiben` kehrt bei
   * abgelaufener Frist ohne Schreiben um: Auf einer kalten Function blieb nach
   * dem Vorlauf der Boden von 1200 ms - und genau dann ging die Stelle
   * verloren, waehrend der letzte Titelanfang mit seiner Null stehenblieb. Ein
   * `PlaybackStopped` hat ueberhaupt nichts zu beantworten, ein Stopp-Befehl
   * nur eine Direktive, die nicht auf die Datenbank wartet. Also wird er
   * unten im `finally` geschrieben, wo Alexa schon nicht mehr wartet -
   * dieselbe Rechnung wie beim Verlauf eine Zeile weiter.
   *
   * Der Preis, damit er nicht geraten werden muss: Das Fenster, in dem sich
   * zwei Ereignisse ueberholen koennen, wird um diese paar Millisekunden
   * groesser. Die Sperren in `standMerken` vergleichen Titel und Stelle, nicht
   * die Reihenfolge - deshalb ist das verkraftbar.
   *
   * `stelle`, `woher` und `gangart` sind fuer den Verlauf: Sie beantworten
   * genau die Frage, die im gemeldeten Fall niemand beantworten konnte -
   * welche Sekunde hinausging und aus welcher Quelle.
   */
  const notiz = { nachspiel: null, stelle: null, woher: null, gangart: null };
  ...
  try {
    ...
    if (typ === 'IntentRequest') {
      ...
      switch (intent.name) {
        ...
        case 'SuchePlaylistIntent':
          return await handlePlay(intent, res, playlists, direktiven, redis, rest, notiz);
        case 'ListPlaylistsIntent':
          return handleList(res, playlists, direktiven);
        case 'AMAZON.PauseIntent':
        case 'AMAZON.StopIntent':
        case 'AMAZON.CancelIntent':
          return haltUndMerken(body, res, playlists, redis, true, notiz);
    ...
    if (typ === 'AudioPlayer.PlaybackFinished') return titelZuEnde(body, res, playlists, redis, notiz);
    ...
    if (typ === 'PlaybackController.PauseCommandIssued') return haltUndMerken(body, res, playlists, redis, false, notiz);
    ...
    // **Erst hier, und das mit Absicht.** Die Antwort ist raus, Alexa wartet
    // nicht mehr - der gemerkte Stand und der Verlauf kosten sie also keine
    // Millisekunde mehr. Und der Verlauf traegt jetzt, was hinausging: Ein
    // `keine Direktive` beim Titelwechsel ist eine andere Diagnose als ein
    // `Play ENQUEUE`, dem Alexa hinterher widerspricht.
    let gemerkt = null;
    if (notiz.nachspiel) {
      // Was hier schiefgeht, darf die Antwort nicht nachtraeglich zu einem
      // Fehler machen - sie ist laengst heraus. Dieselbe Haltung wie beim
      // Verlauf: eine Auskunft, kein Auftrag.
      try {
        gemerkt = await notiz.nachspiel(() => NACHSPIEL_FRIST_MS);
      } catch (err) {
        gemerkt = `nicht gemerkt: ${err?.message || err}`;
 ...
 */
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity, notiz = {}) {
  if (playlists.length === 0) {
    return speak(res, 'Es ist noch keine Playlist angelegt. Bitte lege im Dashboard eine an.', true);
  }
 ...
 */
function titelZuEnde(body, res, playlists, redis, notiz = {}) {
  const lage = laufendes(body, playlists);
  if (!lage) return still(res);
  const ziel = schritt(lage.playlist, lage.token, +1);
  ...
  if (!gehtWeiter && setztFort(lage.playlist)) {
    // Auch dieser Vermerk gehoert hinter die Antwort: `PlaybackFinished` hat
    // nichts zu beantworten, und unter dem Antwortbudget ging er auf einer
    // kalten Function stillschweigend verloren.
    notiz.nachspiel = async (frist) => {
      await standSchreiben(redis, lage.playlist.name, { position: 0, runde: 0, seed: 0, offset: 0, fertig: true }, frist());
      return 'Durchlauf vermerkt';
    };
  }
  return still(res);
}
