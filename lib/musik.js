...
//
// **Fuer einen Start gilt das ausdruecklich nicht.** #137 hat die Runde auch
// dort hochgezaehlt, wenn der neue Token dem des Geraets glich - gegen
// denselben stummen Start, den #138 dann mit einer frischen Adresse behoben
// hat. Geholfen hat die Erhoehung nie (der Log vom 21.9., 20:44-20:47, lief
// mit ihr und blieb zweimal von dreimal still), gekostet hat sie zwei Dinge:
// `selberTitel` vergleicht die Runde mit, also fand `standNichtZurueck` den
// gemerkten Stand nach einem Neustart nicht wieder und ein `PlaybackStarted`
// mit 0 ms hat ihn geloescht - und `weiter` fand ihn aus demselben Grund auch
// nicht. Beides ist wieder heraus; beides steht unter Test.
...
import {
  ...
  verlaufSchreiben, verlaufLesen, verlaufLoeschen, frischeAdresse,
} from './naston.js'
...
export function validierePlaylist(body, bisher = null, basis = '') {
  ...
  // Ein Name, der sich nicht sagen laesst, laesst sich auch nicht finden - und
  // stuende in `findePlaylist` jeder anderen im Weg (siehe dort).
  if (!normalisiere(name)) return { fehler: 'Name muss Buchstaben oder Ziffern enthalten' };
...
/**
 * Bringt die Titel einer Playlist in die uebergebene Reihenfolge.
 *
 * Dieselben Regeln wie `sortierePlaylists`, nur ueber die URL statt den
 * Namen: Unbekannte Adressen werden uebergangen, fehlende behalten ihre
 * Reihenfolge und haengen sich hinten an. Zwischen dem Oeffnen der Liste und
 * dem Speichern kann die Playlist neu gespeichert worden sein - dabei darf
 * kein Titel verlorengehen. Alles andere an der Playlist bleibt, wie es ist.
 *
 * @param {object} playlist
 * @param {string[]} urls die gewuenschte Reihenfolge
 */
export function sortiereTitel(playlist, urls) {
  const gewuenscht = Array.isArray(urls) ? urls : [];
  const offen = [...(playlist.titel || [])];
  const sortiert = [];

  for (const roh of gewuenscht) {
    const url = String(roh ?? '').trim();
    if (!url) continue;
    const i = offen.findIndex(t => t.url === url);
    if (i > -1) sortiert.push(...offen.splice(i, 1));
  }
  return { ...playlist, titel: [...sortiert, ...offen] };
}

/**
 * Der "Weiterhoeren"-Stand nach einer Umsortierung der Titel.
 *
 * **Warum das noetig ist.** Der Stand merkt sich eine Position in der
 * Abspielfolge, keine Adresse. Nach dem Umsortieren zeigte dieselbe Position
 * auf einen anderen Titel, und "weiter" setzte mitten in einem fremden ein.
 * Hier wird der Titel gesucht, auf den der Stand zeigte, und seine neue
 * Position ueber dieselbe Mischung (`reihenfolge` mit gleichem Seed und
 * gleicher Anzahl) zurueckgerechnet - so bleibt es auch bei einer gemischten
 * Playlist derselbe Titel. Offset, Runde und Seed bleiben stehen.
 *
 * @returns {object|null} der neue Stand, `null` wenn es nichts umzurechnen gibt
 */
export function standNachUmsortieren(stand, alt, neu) {
  if (!stand || !Number.isInteger(stand.position)) return null;
  const anzahl = alt.titel.length;
  if (anzahl !== neu.titel.length || stand.position < 0 || stand.position >= anzahl) return null;
  const seed = Number.isInteger(stand.seed) ? stand.seed : 0;
  const folge = reihenfolge(anzahl, seed);
  const url = alt.titel[folge[stand.position]]?.url;
  const nummer = neu.titel.findIndex(t => t.url === url);
  if (nummer === -1) return null;
  const position = folge.indexOf(nummer);
  return position === stand.position ? null : { ...stand, position };
}
...
export function findePlaylist(playlists, slot) {
  ...
  // Ein Name ohne Buchstaben und Ziffern hat die Vergleichsform '' - und die
  // steckt in jedem Text. Er kaeme sonst im letzten Schritt bei jeder Eingabe
  // durch.
  const vergleichbar = liste
    .map(p => ({ p, n: normalisiere(p.name) }))
    .filter(e => e.n);
  // **Beim Enthalten gewinnt der laengste Name, nicht der erste.** Sonst
  // startet "die Kinderlieder" die Playlist "Lieder", sobald die in der Liste
  // weiter oben steht - der kuerzere Name steckt im laengeren ja immer mit drin.
  const enthalten = () => vergleichbar
    .filter(e => e.n.includes(ziel) || ziel.includes(e.n))
    .sort((a, b) => b.n.length - a.n.length)[0]?.p;
  const treffer = liste.find(p => p.name.toLowerCase() === gesagt.toLowerCase())
    || (ziel && vergleichbar.find(e => e.n === ziel)?.p)
    || (ziel && vergleichbar.find(e => e.n.startsWith(ziel))?.p)
    || (ziel && enthalten())
    || null;
...
/**
 * Token eines Streams: `<playlist>|<position>|<runde>|<seed>[|<versuch>[|<pech>]]`.
 *
 ...
 * `pech` zaehlt die Titel, die hintereinander nicht angelaufen sind (siehe
 * `PECH_GRENZE`), und steht nur da, wenn es eine Straehne gibt.
 */
...
/**
 * Ein Seed, dessen Mischung mit Titel `nummer` beginnt.
 *
 * Fuer "Zufallswiedergabe an" mitten im Lied: Das Lied laeuft weiter, und
 * alle anderen kommen danach in dieser Runde - stuende es in der neuen Folge
 * an Stelle sieben, fielen die sechs davor bis zur naechsten Runde aus, ohne
 * Wiederholung ganz. Die Folge muss sich aus dem Seed allein ergeben (siehe
 * `reihenfolge`), also wird gewuerfelt, bis einer passt: im Mittel `anzahl`
 * Versuche, bei hoechstens 200 Titeln eine Sache von Millisekunden. Passt
 * nach der Grenze keiner, gilt der letzte - dann steht der Titel eben
 * woanders, was der Aufrufer ueber `indexOf` ohnehin findet.
 */
export function seedMitVorn(anzahl, nummer, versuche = 5000) {
  let seed = neuerSeed();
  for (let i = 0; i < versuche && reihenfolge(anzahl, seed)[0] !== nummer; i++) seed = neuerSeed();
  return seed;
}
 ...
 *
 * **`stempel` macht die Adresse je Direktive einmalig** - warum, steht bei
 * `frischeAdresse` in lib/naston.js. Er ist hereingereicht und nicht hier
 * gewuerfelt, damit diese Funktion rein bleibt und pruefbar.
 */
export function playDirektive(playlist, position, runde, { seed = 0, verhalten = 'REPLACE_ALL', offset = 0, vorherigerToken, versuch = 0, pech = 0, stempel = Date.now() } = {}) {
  const { titel, nummer } = titelAn(playlist, position, seed);
  const stream = {
    // Fremde MP3-Adressen bleiben unangetastet; nur die eigenen bekommen das
    // `n`, und ein `REPLACE_ALL` dazu die Marke, die eine neue Wiedergabe
    // ausruft (siehe `frischeAdresse` und `laufMarke` in lib/naston.js).
    url: frischeAdresse(titel.url, stempel, verhalten !== 'ENQUEUE'),
...
export async function handleManage(req, res, redis) {
  ...
  if (req.method === 'POST') {
    ...
    // Die Titel einer Playlist umsortieren - wie oben kein Upsert: Es kommt
    // nur die gewuenschte Reihenfolge der URLs. Anders als ein Speichern ueber
    // das Formular nimmt dieser Weg den "Weiterhoeren"-Stand mit.
    if (query.titelSortieren) {
      const name = String(req.body?.name || '').trim().toLowerCase();
      const index = playlists.findIndex(p => p.name.toLowerCase() === name);
      if (index === -1) return res.status(404).json({ error: 'Unknown playlist' });
      const alt = playlists[index];
      const neu = sortiereTitel(alt, req.body?.urls);
      playlists[index] = neu;
      await redis.set(REDIS_KEY, playlists);
      await standUmsortieren(redis, alt, neu);
      return res.status(200).json({ success: true, playlists });
    }
  ...
  if (req.method === 'DELETE') {
    ...
    const uebrig = playlists.filter(p => p.name.toLowerCase() !== name.toLowerCase());
    // Ein Name, den es nicht gibt, ist ein Fehler und kein Erfolg: Ein Skript
    // mit Tippfehler bekam sonst `success: true` und merkte nichts. Dieselbe
    // Antwort wie bei der Pruefung einer unbekannten Playlist.
    if (uebrig.length === playlists.length) return res.status(404).json({ error: 'Unknown playlist' });
    await redis.set(REDIS_KEY, uebrig);
    return res.status(200).json({ success: true, playlists: uebrig });
  }
...
/**
 * Rechnet den Stand einer Playlist nach dem Umsortieren ihrer Titel um.
 *
 * Nicht ueber `standSchreiben`: Das setzt `zeit` neu, und nach `zeit` sucht
 * "weiterhoeren" ohne Namen die zuletzt gehoerte Playlist aus. Eine
 * Umsortierung im Dashboard ist kein Hoeren. Scheitert es, ist die
 * Reihenfolge trotzdem gespeichert - das Schlimmste ist ein "weiter" auf
 * einem anderen Titel.
 */
async function standUmsortieren(redis, alt, neu) {
  try {
    const alle = await redis.get(STAND_KEY);
    const schluessel = alt.name.toLowerCase();
    const umgerechnet = standNachUmsortieren(alle?.[schluessel], alt, neu);
    if (!umgerechnet) return;
    alle[schluessel] = umgerechnet;
    await redis.set(STAND_KEY, alle);
  } catch (err) {
    console.warn('Stand nicht umsortierbar:', err);
  }
}

/**
 * Vergisst den Stand einer Playlist ("von vorn").
 *
 * Mit Frist wie `standSchreiben`: Es laeuft vor der Antwort, und eine
 * haengende Datenbank darf aus "von vorn" keine Stille machen. Laeuft die
 * Frist ab, bleibt der Stand stehen - das ist der kleinere Schaden.
 */
async function standLoeschen(redis, name, frist = Infinity) {
  try {
    const gelesen = await mitFrist(redis.get(STAND_KEY), AUSGEFALLEN, STAND_KEY, frist);
 ...
 * mehr gibt, kommen nicht in Frage; durchgelaufene nur, wenn keine
 * unterbrochene da ist.
 */
async function juengsterStand(redis, playlists, frist = Infinity) {
  ...
  for (const playlist of playlists) {
    ...
    // **Eine unterbrochene Playlist schlaegt jede durchgelaufene**, auch eine
    // juengere: Die durchgelaufene hat nichts fortzusetzen, "weiter" finge sie
    // nur von vorn an. Nur wenn es gar keine unterbrochene gibt, kommt sie
    // zum Zug - von vorn ist dann besser als gar nichts.
    const offen = !stand.fertig;
    const besser = !beste
      || (offen && !beste.offen)
      || (offen === beste.offen && zeit > beste.zeit);
    if (besser) beste = { playlist, stand, zeit, offen };
  }
  return beste;
}
 ...
 * deckte die echte Stelle zu.
 *
 * Die 1093 ms sind dabei keine Fehlmeldung des Geraets, sondern eine ehrliche
 * Auskunft: Im Verlauf steht eine ganze Kette von Wiedergaben, die nach 8, 11,
 * 21 und 42 Sekunden endeten, bei Titeln von rund einer Stunde. Wo die
 * Wiedergabe nach einer Sekunde abbricht, *ist* die letzte bekannte Stelle
 * eine Sekunde. Nur taugt sie als Ziel nichts - `einstieg()` macht 0 daraus -,
 * und sie darf deshalb nicht die zehnte Minute loeschen, die vorher dastand.
 * Ein Fehlversuch, der sofort stirbt, ist keine Auskunft ueber den Abend.
 *
 ...
 * Weiterhoeren zehn Sekunden ab, und wer nach einem Sprung zurueck bei 0:30
...
/**
 * Was das Geraet laut Anfrage gerade haelt - fuer das Log, in einer Klammer.
 *
 * **Warum das hier steht.** Zweimal hintereinander war aus dem Log nicht zu
 * entscheiden, woran ein stummer Start lag: Die Zeile `spielt … ab 2/13 bei
 * 0 ms` nennt Playlist, Stelle, Offset und Gangart - aber nicht den Token,
 * den der Echo bekommt, nicht den, den er noch traegt, und nicht, ob er
 * gerade spielt, pausiert oder nichts tut. Beides musste aus dem Verhalten
 * erschlossen werden, und einmal war der Schluss falsch. Diese drei Werte
 * machen aus der naechsten Reproduktion einen Beweis statt eines Indizes.
 */
function geraeteLage(body, neuerToken) {
  const a = body?.context?.AudioPlayer || {};
  const alt = typeof a.token === 'string' && a.token ? a.token : '-';
  return `Token ${neuerToken}`
    + ` (Geraet ${a.playerActivity || 'ohne Auskunft'} @${a.offsetInMilliseconds ?? '-'} ms`
    + ` mit ${alt}${alt !== '-' && alt === neuerToken ? ' – gleich!' : ''})`;
}

/**
 * Der Token, den das Geraet noch traegt - roh, ohne Deutung.
 *
 * Er geht ins Log und in den Verlauf. Anders als `laufendes` wird hier nichts
 * geprueft und nichts nachgeschlagen: Auch ein Token aus einer geloeschten
 * Playlist oder aus einer aelteren Fassung ist eine Auskunft ueber den
 * Zustand des Geraets, und beim Nachsehen zaehlt genau die.
 */
function geraeteToken(body) {
  const roh = body?.context?.AudioPlayer?.token;
...
//
// **Die Loesung.** Der Echo bekommt keine Adresse mit Sitzungsnummer mehr,
// sondern eine dieser App (`/api/skill?ton=…`, siehe lib/naston.js). Die
// Nummer braucht nur noch der Durchleiter, der die Datei bei der Box holt -
// und er fragt sie hier ab (`fritzSid`), waehrend der Echo auf Bytes wartet.
// Frueher wurde `sid=` vor jeder Antwort in die Adressen getauscht; das ist
// mit dem Durchleiter weggefallen.
//
 ...
 *
 * `erneuert` sagt dem Aufrufer, ob sich die Nummer durch Stufe 3 wirklich
 * geaendert hat; `angemeldet`, ob Stufe 3 ueberhaupt gelaufen ist (siehe
 * unten).
 *
...
/**
 * Die gemerkte Tafel `{ [link]: {sid, zeit} }`, oder `{}`.
 *
 * Beide frueheren Formen unter diesem Schluessel werden gelesen - siehe
 * `alsSidTafel`.
 */
 ...
 * das so, und zehn Sekunden sind der Wert, mit dem der Satz auch dann wieder
 * anfaengt, wenn zwischen Stopp und gemerkter Stelle eine Sekunde liegt.
 *
 * Nebenbei erledigt der Vorlauf den anderen Rand: Wer nach neun Sekunden
 * stoppt, faengt wieder von vorn an, statt diese neun Sekunden zu
...
function vorlaufMs() {
  ...
  return Number.isFinite(gesetzt) && gesetzt >= 0 ? gesetzt : 10000;
}
...
export async function handleSkill(body, res, redis) {
  ...
  try {
    ...
    if (typ === 'IntentRequest') {
      ...
      switch (intent.name) {
        ...
        case 'AMAZON.StartOverIntent':
          return await vonVorn(body, res, playlists, redis, rest);
    ...
    if (typ === 'AudioPlayer.PlaybackStarted' || typ === 'AudioPlayer.PlaybackStopped') {
      ...
      // Das Gegenstueck fuer die Startwache - ebenfalls vor der Antwort, siehe
      // `startGespielt`.
      if (typ === 'AudioPlayer.PlaybackStarted') await startGespielt(redis, body.request?.token, ereignisFrist());
      return still(res);
    }
    ...
    // **Alexas eigene Fehlerbeschreibung, und sie lag bisher im Papierkorb.**
    // Ein `SessionEndedRequest` fiel hier unbesehen in die leere Antwort. Er
    // traegt aber `reason` - und bei `ERROR` dazu ein `error` mit `type`
    // (`INVALID_RESPONSE`, `DEVICE_COMMUNICATION_ERROR`, `INTERNAL_ERROR`)
    // und `message`. Das ist die einzige Stelle, an der Alexa von sich aus
    // sagt, warum sie eine Antwort nicht verarbeiten konnte. Im Log vom
    // 21.9., 21:49:01, stand genau einer, und wir wussten nichts ueber ihn.
    //
    // Ein `USER_INITIATED` ist dagegen der Normalfall ("Alexa, stopp" im
    // Dialog) und bleibt eine Zeile im Log, ohne den kurzen Verlauf zu
    // fuellen - der soll ein Zeuge fuer Stoerungen sein, kein Mitschnitt.
    if (typ === 'SessionEndedRequest' && sitzungsSchluss(body).auffaellig) {
      const { grund, fehler } = sitzungsSchluss(body);
      console.error(`musik-box Alexa hat die Sitzung beendet: ${grund}`
        + `${fehler ? ` – ${fehler}` : ''}`);
    }
    ...
    const echoLinie = typ.startsWith('AudioPlayer.') || typ === 'System.ExceptionEncountered'
      // Nur der auffaellige Schluss - der gewoehnliche wuerde den kurzen
      // Verlauf mit Selbstverstaendlichkeiten fuellen.
      || (typ === 'SessionEndedRequest' && sitzungsSchluss(body).auffaellig);
    if (echoLinie || wortLinie) {
      await verlaufSchreiben(redis, {
        was: echoLinie ? 'echo' : 'wort',
        ereignis: echoLinie
          ? (typ.startsWith('AudioPlayer.') ? typ.slice('AudioPlayer.'.length)
            : (typ === 'SessionEndedRequest' ? `SessionEnded ${sitzungsSchluss(body).grund}` : 'ExceptionEncountered'))
        ...
        // Nur der Sitzungsschluss bekommt die ausfuehrliche Fassung ("Typ:
        // Text"): Dort ist der Text die ganze Auskunft. Bei den uebrigen
        // Ereignissen bleibt es beim Typ, so wie es war.
        fehler: (typ === 'SessionEndedRequest' ? sitzungsSchluss(body).fehler : null)
          || body.request?.error?.type || body.request?.error?.message || null,
        ...
        // **Die beiden Zeichenketten, an denen die letzte Fehlersuche haengen
        // blieb.** `token` ist der, den der Echo bekommt, `geraet` der, den er
        // schon traegt. Nebeneinander beantworten sie in einer Zeile, was
        // vorher nur der Vercel-Export hergab.
        token: geplatzt ? null : tokenHinaus(hinaus),
        geraet: geraeteToken(body),
      });
    }

    // **Und zuletzt die Wache.** Sie steht hinter dem Verlauf, weil sie von
    // ihm lebt: Was sie meldet, ist ein Eintrag wie jeder andere - nur einer,
    // den bisher niemand geschrieben hat, weil niemand gemerkt hat, dass er
    // fehlt.
    const start = geplatzt ? null : startHinaus(hinaus);
    const stumm = await startWache(redis, {
      neuerStart: start?.token || null,
      seit: beginn,
      beleg: { titel: start?.titel || null, geraet: geraeteToken(body) },
    });
    if (stumm) {
      const her = new Date(Number(stumm.zeit)).toISOString().slice(11, 19);
      console.warn(`musik-box stummer Start um ${her}: ${stumm.token} hat nie gespielt`
        + `${stumm.titel ? ` (${stumm.titel})` : ''}`
        + `${stumm.geraet ? `, das Geraet trug ${stumm.geraet}` : ', das Geraet trug nichts'}`);
      await verlaufSchreiben(redis, {
        was: 'echo',
        ereignis: 'STUMM (kein PlaybackStarted)',
        zeit: Number(stumm.zeit),
        titel: stumm.titel || null,
        token: stumm.token,
        geraet: stumm.geraet || null,
      });
    }
  }
}
...
async function handlePlay(intent, res, body, playlists, direktiven, redis, rest = () => Infinity, notiz = {}) {
  ...
  console.log(`musik-box spielt ${playlist.name} (gehoert: "${gesagt}")`
    ...
    + `; ${geraeteLage(body, spiel.audioItem.stream.token)}`
    + `${vorspann().length ? ', mit ClearQueue' : ''}`);
  ...
  // Aber nur, wenn es eine Sitzung gibt: Ohne sie verwirft Alexa eine Antwort
  // mit `shouldEndSession` ganz - dieselbe Regel wie bei `hatSitzung`.
  if (!sagtAn(playlist)) return still(res, start, ausSprache(body));
 ...
 *
 * **Beide Richtungen springen nicht.** Der laufende Titel spielt an seiner
 * Sekunde weiter; was sich aendert, ist die Folge danach. Beim Einschalten
 * steht er dafuer vorn in einer frischen Mischung (`seedMitVorn`), und alle
 * anderen kommen danach in dieser Runde. Frueher sprang das
 * Einschalten auf den ersten Titel der neuen Folge und das Abschalten auf den
 * ersten der Liste; hoerbar war das, aber wer mitten im Lied war, war raus.
 *
 * Die Runde zaehlt hoch, damit der neue Token sich vom laufenden unterscheidet.
 */
function mischen(body, res, playlists, an) {
  ...
  // Welcher Titel gerade laeuft - als Nummer in der Liste, unabhaengig von
  // der Mischung, in der er gerade steht.
  const passt = lage.token.position < lage.playlist.titel.length;
  if (!passt) {
    // Die Playlist wurde seither gekuerzt: Den laufenden Titel gibt es nicht
    // mehr, also von vorn in der gewuenschten Folge.
    return still(res, [playDirektive(lage.playlist, 0, lage.token.runde + 1, { seed: an ? neuerSeed() : 0 })], ausSprache(body));
  }
  const { nummer } = titelAn(lage.playlist, lage.token.position, lage.token.seed);
  // Ungemischt ist die Stelle die Listennummer. Gemischt steht der Titel vorn
  // in einer frischen Folge, damit alle anderen in dieser Runde noch kommen.
  const seed = an ? seedMitVorn(lage.playlist.titel.length, nummer) : 0;
  const position = an ? reihenfolge(lage.playlist.titel.length, seed).indexOf(nummer) : nummer;
  return still(res, [playDirektive(lage.playlist, position, lage.token.runde + 1, { seed, offset: lage.offset })], ausSprache(body));
}
...
async function weiter(body, res, playlists, direktiven = [], stumm = false, redis = null, rest = () => Infinity, notiz = {}) {
  ...
  if (lage) {
    ...
    console.log(`musik-box weiter ${lage.playlist.name} ab ${position + 1}/${lage.playlist.titel.length}`
      ...
      + `; ${geraeteLage(body, start.audioItem.stream.token)}`);
...
/**
 * Naechster oder voriger Titel - vom Sprachbefehl wie vom Knopf - und ohne
 * Wiederholung nicht ueber die Raender.
 *
...
async function vonVorn(body, res, playlists, redis, rest = () => Infinity) {
  const lage = laufendes(body, playlists);
  ...
  if (setztFort(lage.playlist)) await standLoeschen(redis, lage.playlist.name, rest());
...
/**
 * Was Alexa ueber das Ende einer Sitzung sagt.
 *
 * `USER_INITIATED` ist der Normalfall und faellt nicht auf; alles andere
 * schon - `ERROR` traegt die Beschreibung, `EXCEEDED_MAX_REPROMPTS` heisst,
 * dass eine Rueckfrage ins Leere lief. Rein und exportiert, damit die
 * Einordnung ohne Alexa pruefbar ist.
 */
export function sitzungsSchluss(body) {
  const anfrage = body?.request || {};
  const grund = typeof anfrage.reason === 'string' ? anfrage.reason : 'ohne Angabe';
  const f = anfrage.error;
  const fehler = f ? [f.type, f.message].filter(Boolean).join(': ') || JSON.stringify(f) : null;
  return { grund, fehler, auffaellig: grund !== 'USER_INITIATED' || Boolean(fehler) };
}

// ---------------------------------------------------------------------------
// Die Startwache: ein Start, der nie gespielt hat, soll das selbst sagen
// ---------------------------------------------------------------------------
//
// **Warum es sie gibt.** Ein `Play REPLACE_ALL`, den der Echo stillschweigend
// fallen laesst, hinterlaesst nirgends eine Spur: Der Skill antwortet in
// Millisekunden, der Echo holt die Tondatei und bekommt sie ganz - und dann
// kommt nichts. Kein `PlaybackStarted`, kein `PlaybackFailed`, kein
// `ExceptionEncountered`. Im Log vom 21.9., 21:44-21:49, ist das dreimal
// passiert, und erfahren haben wir es nur, weil jemand daneben stand und es
// gesagt hat: "einmal oder zweimal ging es nicht."
//
// Damit ist jede Messung eine Erzaehlung. Die Wache macht daraus eine Zahl:
// Beim Start wird eine Marke gesetzt, ein `PlaybackStarted` legt seine
// eigene daneben, und die naechste Anfrage vergleicht beide. Fehlt das
// Gegenstueck, steht der stumme Start mit Uhrzeit, Titel und beiden Token im
// Verlauf - dort, wo auch alles andere steht.
//
// **Zwei Marken statt einer, und das ist kein Umweg.** Eine einzelne, die
// das `PlaybackStarted` loescht, haengt an der Reihenfolge: Friert die
// Instanz nach der Antwort ein, kann die Marke des Starts *nach* dem
// Loeschen ankommen und bliebe fuer immer stehen - ein Fehlalarm bei jedem
// geglueckten Start. Zwei unabhaengige Schreibvorgaenge haben diese
// Reihenfolge nicht; das Urteil faellt spaeter aus beiden zusammen.
const START_OFFEN_KEY = 'musik_start_offen';
const START_LIEF_KEY = 'musik_start_lief';
const START_TTL_S = 900;

/**
 * Wie lange ein Start Zeit hat, sich zu melden, bevor er als stumm gilt.
 *
 * Im Log kam das `PlaybackStarted` 1 bis 5 Sekunden nach dem Abruf; fuenfzehn
 * sind reichlich Abstand dazu und immer noch kurz genug, dass der naechste
 * Versuch das Urteil traegt. `MUSIK_STARTWACHE=0` schaltet sie ganz ab.
 */
const STARTFRIST_MS = () => {
  const roh = process.env.MUSIK_STARTWACHE;
  if (roh === '0') return 0;
  const ms = Number(roh);
  return Number.isFinite(ms) && ms > 0 ? Math.max(3000, ms) : 15000;
};

/**
 * Der Token einer Start-Direktive (`REPLACE_ALL`) - oder null.
 *
 * Nur ein Start wird bewacht: Ein `ENQUEUE` haengt sich hinter einen Titel,
 * der noch laeuft, und sein `PlaybackStarted` kommt erst Minuten spaeter.
 */
export function startHinaus(koerper) {
  const liste = koerper?.response?.directives;
  if (!Array.isArray(liste)) return null;
  const play = liste.find(d => d?.type === 'AudioPlayer.Play' && d.playBehavior !== 'ENQUEUE');
  const token = play?.audioItem?.stream?.token;
  return token ? { token, titel: play.audioItem.metadata?.title || null } : null;
}

/**
 * Hat der gemerkte Start gespielt? Verglichen werden Token **und** Zeit.
 *
 * Die Zeit muss mit, weil zwei Starts hintereinander denselben Token tragen
 * koennen - genau so sah der gemeldete Fall aus: stumm um 21:45:20, auf
 * denselben Token noch einmal um 21:45:39, und der spielte. Ohne den
 * Zeitvergleich haette der zweite den ersten entlastet.
 *
 * Rein und exportiert, damit das Urteil ohne Alexa pruefbar ist.
 */
export function startGeklappt(offen, lief) {
  if (!offen) return true;
  if (!lief || !lief.token) return false;
  return lief.token === offen.token && Number(lief.zeit) > Number(offen.zeit);
}

/**
 * Das Gegenstueck zur Startmarke: "dieser Token hat gespielt".
 *
 * **Vor der Antwort, nicht dahinter.** Hier stand es in der Wache hinter der
 * Antwort, und dort friert Vercel die Instanz ein - im Log liefen solche
 * Schreibvorgaenge bis zu 4 Minuten 27 Sekunden spaet. Pruefte in der
 * Zwischenzeit eine andere Instanz, fand sie den Start ohne Gegenstueck und
 * meldete ihn als STUMM, obwohl er spielte. `PlaybackStarted` hat nichts zu
 * beantworten; die leere Antwort kann auf diesen einen Schreibvorgang warten,
 * wie auf den gemerkten Stand.
 */
async function startGespielt(redis, token, frist = Infinity) {
  if (!STARTFRIST_MS() || !redis || !token) return;
  try {
    await mitFrist(
      redis.set(START_LIEF_KEY, { token, zeit: Date.now() }, { ex: START_TTL_S }),
      null, START_LIEF_KEY, frist,
    );
  } catch (err) {
    console.warn('musik-box Startwache nicht schreibbar:', err?.message || err);
  }
}

/**
 * Die Wache: urteilen, aufraeumen, neu merken. Ein Fehler hier kostet nie die
 * Wiedergabe - sie ist eine Auskunft, kein Auftrag, und laeuft deshalb auch
 * erst hinter der Antwort.
 */
async function startWache(redis, { neuerStart = null, jetzt = Date.now(), seit = jetzt, beleg = {} } = {}) {
  const frist = STARTFRIST_MS();
  if (!frist || !redis) return null;
  let gemeldet = null;
  // Upstash gibt zurueck, was es selbst serialisiert hat - ein Eintrag aus
  // einer anderen Fassung kann als Zeichenkette dort liegen. Was sich nicht
  // lesen laesst, gilt als nicht vorhanden.
  const lies = async (key) => {
    const roh = await redis.get(key);
    if (roh && typeof roh === 'object') return roh;
    if (typeof roh === 'string') { try { return JSON.parse(roh); } catch { return null; } }
    return null;
  };
  try {
    const offen = await lies(START_OFFEN_KEY);
    if (offen && jetzt - Number(offen.zeit) >= frist) {
      const lief = await lies(START_LIEF_KEY);
      if (!startGeklappt(offen, lief)) gemeldet = offen;
      await redis.del(START_OFFEN_KEY);
    }
    if (neuerStart) {
      // `seit` ist der Moment der Anfrage, nicht der des Schreibens: Diese
      // Zeile laeuft hinter der Antwort und kann eingefroren Minuten spaet
      // kommen. Mit der spaeten Uhrzeit laege die Marke *hinter* dem
      // `PlaybackStarted`, das inzwischen vor seiner Antwort geschrieben
      // wurde - und der Start galt als stumm, obwohl er spielte.
      await redis.set(START_OFFEN_KEY, { token: neuerStart, zeit: seit, ...beleg }, { ex: START_TTL_S });
    }
  } catch (err) {
    console.warn('musik-box Startwache nicht lesbar:', err?.message || err);
  }
  return gemeldet;
}

/**
 * Der Token der ersten Play-Direktive - der, den der Echo bekommt.
 *
 * **Warum er in den Verlauf gehoert.** Zwei Fehlersuchen hintereinander sind
 * daran haengen geblieben, dass genau diese Zeichenkette nirgends stand: Das
 * Dashboard zeigte Stelle, Gangart und `playerActivity`, der Token stand nur
 * im Log von Vercel - und wer Musik hoert, exportiert kein Vercel-Log. Beide
 * Male musste aus dem Verhalten geschlossen werden, und einmal war der
 * Schluss falsch.
 */
function tokenHinaus(koerper) {
  const liste = koerper?.response?.directives;
  if (!Array.isArray(liste)) return null;
  const play = liste.find(d => d?.type === 'AudioPlayer.Play');
  return play?.audioItem?.stream?.token || null;
}
...
async function nachFehler(body, res, playlists, rest = () => Infinity) {
  ...
  // **Die Zeile nennt, was wirklich geschieht - und steht deshalb erst nach
  // der Entscheidung.** Vorher stand hier vorab "zweiter Versuch", sobald der
  // Zaehler auf null stand - auch wenn es den Titel in der gekuerzten Liste
  // gar nicht mehr gab und stattdessen weitergesprungen wurde, und "weiter mit
  // dem naechsten Titel" auch dann, wenn die Wiedergabe endete.
  const melde = (folge) => console.warn('Alexa konnte nicht abspielen:', fehler.type, fehler.message,
    'Token:', body.request.token,
    'Offset:', offsetMs,
    wer ? `Titel: ${wer}` : '',
    `- ${folge}`);
  if (!lage) {
    melde('Playlist unbekannt, nichts zu tun');
    return still(res);
  }
