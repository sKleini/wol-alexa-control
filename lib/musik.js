...
function handlePlay(intent, res, playlists, direktiven) {
  ...
  const anzahl = playlist.titel.length === 1 ? 'einen Titel' : `${playlist.titel.length} Titel`;
  ...
  return speak(res, `Ich spiele ${playlist.name}, ${anzahl}.`, true, [playDirektive(playlist, 0, 0)]);
}
