...
export async function handleSkill(body, res, redis) {
  ...
  try {
    ...
    // nicht mehr - der Verlauf kostet sie also keine Millisekunde mehr. Und
    // er traegt jetzt, was hinausging: Ein `keine Direktive` beim
    // Titelwechsel ist eine andere Diagnose als ein `Play ENQUEUE`, dem
    // Alexa hinterher widerspricht.
    if (typ.startsWith('AudioPlayer.') || typ === 'System.ExceptionEncountered') {
      await verlaufSchreiben(redis, {
        was: 'echo',
        ereignis: typ.startsWith('AudioPlayer.') ? typ.slice('AudioPlayer.'.length) : 'ExceptionEncountered',
...
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity) {
  ...
  console.log(`musik-box spielt ${playlist.name} (gehoert: "${gesagt}")`
    ...
    + ` bei ${offset} ms: ${spiel.audioItem.metadata.title}`);
