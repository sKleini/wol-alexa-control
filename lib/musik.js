...
//
// **Und dasselbe gilt fuer einen Start.** Ein `REPLACE_ALL` mit dem Token, den
// das Geraet noch traegt, bleibt genauso stumm wie ein `ENQUEUE` damit - das
// ist der gemeldete Fehler "starten, kurz darauf stoppen, wieder starten, und
// es kommt nichts". Darum bekommt jede Start-Direktive den Token des Geraets
// als `ungleich` mit; die Begruendung steht bei `playDirektive`.
 ...
 *
 * **`ungleich` ist der Token, den das Geraet noch traegt - und der Grund, aus
 * dem eine Playlist stumm bleiben kann.** Gemeldet war: eine Ordnerfreigabe
 * starten, kurz darauf stoppen, wieder starten - und es kommt kein Ton. Im
 * Log steht der Fall in drei Zeilen: `spielt Udo CD eins ab 1/13 bei 0 ms`,
 * der Echo holt die Datei tatsaechlich ab (`GET /api/skill?ton=...`), und
 * dann nichts mehr - kein `PlaybackStarted`, kein `PlaybackFailed`, keine
 * Stelle. Der Unterschied zu jedem Start, der geklappt hat, ist genau einer:
 * Diesmal war der neue Token **Zeichen fuer Zeichen derselbe** wie der des
 * Stroms, den das Geraet noch hielt (`Udo CD eins|0|0|0`). Wer kurz nach dem
 * Start stoppt, steht ja noch beim ersten Titel, also fallen Stelle, Runde
 * und Mischung des neuen Tokens mit denen des alten zusammen; wer laenger
 * hoert, bekommt durch die weitergezaehlte Stelle von selbst einen anderen.
 *
 * Dass Alexa einen gleichlautenden Token nicht als neuen Strom annimmt, steht
 * in diesem Modul schon fuer `ENQUEUE` (siehe Kopf der Datei) - fuer
 * `REPLACE_ALL` galt es unausgesprochen genauso. Ein `ClearQueue` davor
 * aendert daran nichts; das raeumt die Warteschlange, nicht die Erinnerung an
 * den laufenden Titel.
 *
 * **Die Runde ist der Knopf dafuer**, und sie war es hier immer schon: "die
 * Runde zaehlt hoch, damit der neue Token nie dem laufenden gleicht" steht
 * ueber der Wiederholung, und `mischen` und `vonVorn` machen es von Hand.
 * Hier wird daraus eine Regel, die keiner mehr vergessen kann.
 */
export function playDirektive(playlist, position, runde, { seed = 0, verhalten = 'REPLACE_ALL', offset = 0, vorherigerToken, versuch = 0, pech = 0, ungleich = null, stempel = Date.now() } = {}) {
  const { titel, nummer } = titelAn(playlist, position, seed);
  // Nur fuer einen Start: Ein `ENQUEUE` haengt hinter den laufenden Titel und
  // weist sich ueber `expectedPreviousToken` aus - dort ist der Token des
  // Geraets die Zusage und nicht die Kollision.
  let eigeneRunde = runde;
  if (verhalten !== 'ENQUEUE' && ungleich
    && tokenBauen(playlist.name, position, eigeneRunde, seed, versuch, pech) === ungleich) {
    eigeneRunde += 1;
  }
  const stream = {
    // **Je Direktive eine eigene Adresse** - warum, steht bei `frischeAdresse`
    // in lib/naston.js: Ein anderer Token allein hat nicht gereicht. Fremde
    // MP3-Adressen bleiben unangetastet.
    url: frischeAdresse(titel.url, stempel),
    token: tokenBauen(playlist.name, position, eigeneRunde, seed, versuch, pech),
 ...
 *
 * Er geht als `ungleich` in jede Start-Direktive: Nur daran laesst sich
 * erkennen, ob der neue Strom fuer Alexa derselbe waere wie der alte. Anders
 * als `laufendes` wird hier nichts geprueft - auch ein Token aus einer
 * geloeschten Playlist oder aus einer fremden Fassung ist einer, mit dem der
 * neue nicht zusammenfallen darf.
 */
...
async function handlePlay(intent, res, body, playlists, direktiven, redis, rest = () => Infinity, notiz = {}) {
  ...
  const spiel = playDirektive(playlist, position, runde, { seed, offset, ungleich: geraeteToken(body) });
...
async function weiter(body, res, playlists, direktiven = [], stumm = false, redis = null, rest = () => Infinity, notiz = {}) {
  ...
  if (lage) {
    ...
    const start = playDirektive(lage.playlist, position, lage.token.runde,
      { seed: lage.token.seed, offset, ungleich: geraeteToken(body) });
  ...
  if (juengste) {
    ...
    const start = playDirektive(juengste.playlist, ab.position, ab.runde,
      { seed: ab.seed, offset: ab.offset, ungleich: geraeteToken(body) });
...
function springe(body, res, playlists, richtung) {
  ...
  if (ziel.umbruch && !wiederholtSich(lage.playlist)) {
    ...
    return still(res, [playDirektive(lage.playlist, 0, lage.token.runde,
      { seed: lage.token.seed, ungleich: geraeteToken(body) })], schluss);
  }
  return still(res, [playDirektive(lage.playlist, ziel.position, ziel.runde,
    { seed: ziel.seed, ungleich: geraeteToken(body) })], schluss);
}
