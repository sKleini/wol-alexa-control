...
export async function handleSkill(body, res, redis) {
  ...
  try {
    ...
    const stumm = await startWache(redis, {
      ...
      gespielt: typ === 'AudioPlayer.PlaybackStarted' ? (body.request?.token || null) : null,
 ...
 * Das Abschalten springt dagegen nicht: Es spielt den laufenden Titel an
 * seiner Stelle weiter, nur ohne Mischung danach.
 */
function mischen(body, res, playlists, an) {
  ...
  if (!an) {
    // **Abschalten springt nicht auf Titel eins.** Die Mischung faellt weg,
    // aber der laufende Titel bleibt, an seiner Stelle: Ungemischt ist die
    // Stelle in der Folge seine Nummer in der Liste, und von dort geht es in
    // Listenreihenfolge weiter. Frueher war es Stelle 0 - also der erste Titel
    // der Liste, und wo man gerade war, war weg.
    const passt = lage.token.position < lage.playlist.titel.length;
    const nummer = passt ? titelAn(lage.playlist, lage.token.position, lage.token.seed).nummer : 0;
    const offset = passt ? lage.offset : 0;
    return still(res, [playDirektive(lage.playlist, nummer, lage.token.runde + 1, { seed: 0, offset })], ausSprache(body));
  }
  return still(res, [playDirektive(lage.playlist, 0, lage.token.runde + 1, { seed: neuerSeed() })], ausSprache(body));
}
 ...
 */
async function startWache(redis, { neuerStart = null, gespielt = null, jetzt = Date.now(), beleg = {} } = {}) {
  ...
  try {
    ...
    if (gespielt) await redis.set(START_LIEF_KEY, { token: gespielt, zeit: jetzt }, { ex: START_TTL_S });
    if (neuerStart) {
      await redis.set(START_OFFEN_KEY, { token: neuerStart, zeit: jetzt, ...beleg }, { ex: START_TTL_S });
    }
  } catch (err) {
