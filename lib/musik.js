...
async function fritzAufgefrischt(playlists, body, redis, rest = () => Infinity) {
  if (!playlists.some(p => p.quelle?.typ === 'fritz')) return { playlists, fritzOk: true, fritzErneuert: false, fritzUngeprueft: false };

  const gemeint = gemeintePlaylist(playlists, body);
  if (gemeint?.quelle?.typ !== 'fritz') return { playlists, fritzOk: true, fritzErneuert: false, fritzUngeprueft: false };
  ...
  const { playlist: frisch, verlaesslich, erneuert, ungeprueft } = await mitFrischerSid(gemeint, redis, erzwingen, rest);
  return {
    playlists: playlists.map(p => (p === gemeint ? frisch : p)),
    fritzOk: verlaesslich,
    fritzErneuert: erneuert,
    fritzUngeprueft: ungeprueft,
  };
}
...
async function mitFrischerSid(playlist, redis, erzwingen = false, rest = () => Infinity) {
  if (playlist?.quelle?.typ !== 'fritz') return { playlist, verlaesslich: true, erneuert: false, ungeprueft: false };
  const { sid, verlaesslich, erneuert, ungeprueft } = await fritzSid(playlist.quelle.link, redis, erzwingen, rest);
  ...
  if (!sid) return { playlist, verlaesslich: false, erneuert: false, ungeprueft: false };
  return {
...
async function fritzSid(link, redis, erzwingen = false, rest = () => Infinity) {
  ...
  if (!freigabe) return { sid: null, verlaesslich: false, erneuert: false, ungeprueft: false };
  ...
  if (!erzwingen && passt) return { sid: eigene, verlaesslich: true, erneuert: false, ungeprueft: false };
  ...
  if (eigene && rest() >= SID_PRUEF_MINDESTBUDGET_MS) {
    ...
    if (gilt === true) {
      ...
      return { sid: eigene, verlaesslich: true, erneuert: false, ungeprueft: false };
    }
    ...
    if (gilt === null && passt) return { sid: eigene, verlaesslich: true, erneuert: false, ungeprueft: false };
  } else if (passt) {
  ...
  if (rest() < SID_MINDESTBUDGET_MS) {
    ...
    return { sid: eigene, verlaesslich: false, erneuert: false, ungeprueft: !gefragt };
  }
  ...
  if (!ergebnis.sid) {
    ...
    return { sid: eigene, verlaesslich: false, erneuert: false, ungeprueft: false };
  }
  await fritzSidMerken(redis, link, ergebnis.sid);
  return { sid: ergebnis.sid, verlaesslich: true, erneuert: ergebnis.sid !== eigene, ungeprueft: false };
}
...
export async function handleSkill(body, res, redis) {
  ...
  const { fritzOk, fritzErneuert, fritzUngeprueft } = aufgefrischt;
    ...
    if (typ === 'IntentRequest') {
      ...
      switch (intent.name) {
        ...
        case 'SuchePlaylistIntent':
          return await handlePlay(intent, res, playlists, direktiven, redis, rest, fritzOk, fritzUngeprueft);
 ...
 */
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity, fritzOk = true, fritzUngeprueft = false) {
  if (playlists.length === 0) {
    return speak(res, 'Es ist noch keine Playlist angelegt. Bitte lege im Dashboard eine an.', true);
  }
