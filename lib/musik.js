...
import {
  ...
  lesbareDauer, laengerAlsSitzung, adresseKurz, datenrateKbs,
} from './groesse.js'
export {
  groesseAusContentRange, lesbareGroesse, spieldauerSekunden,
  lesbareDauer, laengerAlsSitzung, adresseKurz, datenrateKbs,
...
export async function handleManage(req, res, redis) {
  ...
  if (req.method === 'GET') {
    ...
    if (req.query.pruefen) {
      ...
      const frisch = await mitFrischerSid(playlist, redis);
 ...
 *
 * `frischeSid` raeumt sich selbst vier Sekunden ein (SID_MS in
 * lib/fritznas.js) und braucht davor noch den Erlaubnis-Check. Wer mit weniger
 * startet, laeuft mit hoher Wahrscheinlichkeit in Alexas Abbruch statt in eine
 * gueltige Nummer.
 */
const SID_MINDESTBUDGET_MS = 4800;
...
async function fritzAufgefrischt(playlists, body, redis, rest = () => Infinity) {
  ...
  const frisch = await mitFrischerSid(gemeint, redis, erzwingen, rest);
  // **Unveraendert zurueck heisst: keine Nummer bekommen.** mitFrischerSid baut
  // sonst ein neues Objekt mit ausgetauschtem `sid`. Dieselbe Unterscheidung
  // benutzt "Check URLs" fuer seinen Hinweis - hier entscheidet sie, ob der
  // Skill ueberhaupt behaupten darf, er spiele etwas.
  return {
    playlists: playlists.map(p => (p === gemeint ? frisch : p)),
    fritzOk: frisch !== gemeint,
  };
}
...
async function mitFrischerSid(playlist, redis, erzwingen = false, rest = () => Infinity) {
  if (playlist?.quelle?.typ !== 'fritz') return playlist;
  const sid = await fritzSid(playlist.quelle.link, redis, erzwingen, rest);
  if (!sid) return playlist;
  return { ...playlist, titel: playlist.titel.map(t => ({ ...t, url: mitSid(t.url, sid) })) };
}
...
async function fritzSid(link, redis, erzwingen = false, rest = () => Infinity) {
  ...
  if (!erzwingen && passt) return gemerkt.sid;
  ...
  // die ganze Wiedergabe statt nur einen Titel. Mit der alten Nummer
  // loszuspielen ist der bessere Handel: Gilt sie noch, merkt es niemand; gilt
  // sie nicht, scheitert der erste Titel - und PlaybackFailed holt dann eine
  // frische Nummer (siehe `erzwingen` in fritzAufgefrischt) und spielt weiter.
  // Aus Schweigen wird so schlimmstenfalls eine kleine Verzoegerung.
  if (rest() < SID_MINDESTBUDGET_MS) {
    console.warn(`FRITZ!NAS-Login ausgelassen, nur noch ${rest()} ms Budget`);
    return gemerkt?.link === link ? gemerkt.sid : null;
  }

  const ergebnis = await frischeSid(freigabe);
  if (!ergebnis.sid) {
    console.error(`FRITZ!NAS-Sitzung fuer ${link}: ${ergebnis.fehler}`);
    // Als letzter Versuch die gemerkte - aber nur, wenn sie ueberhaupt zu
    // dieser Freigabe gehoert. Die einer anderen waere nicht "vielleicht noch
    // gut", sondern sicher falsch.
    return gemerkt?.link === link ? gemerkt.sid : null;
  }
