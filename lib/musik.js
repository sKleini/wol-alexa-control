...
/**
 * Was eine vollstaendige Anmeldung kostet - gemessen, nicht geschaetzt.
 *
 * Zwei Wege zur Box: das Geruest holen und die Nummer gegenpruefen. Im
 * Betrieb zwischen 1443 und 2635 ms. Passt das nicht mehr ins Eilziel, faellt
 * die Gegenprobe weg (siehe `frischeSid`): Sie ist die entbehrlichere Haelfte,
 * und ein Irrtum traegt sich ueber `PlaybackFailed` selbst.
 */
const LOGIN_VOLL_MS = 2600;

/**
 * Vor dem ersten Ton wird nicht angemeldet - und das ist keine Frage der Zeit.
 *
 * **Vier Logs, sieben Wiedergabestarts, eine Spalte, die alles erklaert.**
 * Gezaehlt wurde, ob im selben Request eine Anmeldung lief und ob danach
 * ueberhaupt ein AudioPlayer-Ereignis kam:
 *
 *   wartet  Anmeldung  Ergebnis
 *     1122     nein     PlaybackFailed → PlaybackStarted
 *     1569     nein     PlaybackStarted
 *     1737     nein     PlaybackFailed → PlaybackStarted
 *     1689     **ja**   nichts
 *     2003     **ja**   PlaybackFailed
 *     2539     **ja**   nichts
 *     2857     **ja**   nichts
 *
 * Ohne Anmeldung spielte es dreimal von drei. Mit Anmeldung kam dreimal von
 * vier **gar kein Ereignis** - der Echo hat die Direktive nicht einmal
 * versucht. Und der Fall bei 1689 ms ist der, der die bisherige Erklaerung
 * kippt: Die Antwort war schnell, die Nummer frisch geholt und gueltig, und
 * es blieb trotzdem still. Die Zeit trennt diese Faelle nicht, die Anmeldung
 * tut es.
 *
 * Was die Box in diesen Sekunden tut, ist von aussen nicht zu sehen; dass
 * `filelink.lua` ohne Sitzung alle Sitzungen der Box beendet, schreibt AVM
 * selbst. Der Abstand nach der Anmeldung und der zweite Weckruf waren dagegen
 * gebaut und fallen vor dem ersten Ton regelmaessig der Eile zum Opfer - was
 * bleibt, ist, die Anmeldung dorthin zu verlegen, wo sie nichts kostet.
 *
 * **Und das ist der Fehlerweg.** Startet der Skill mit der Nummer, die er
 * hat - und sei sie tot -, meldet der Echo `PlaybackFailed`. Dieser Request
 * hat frische acht Sekunden, eine warme Function, keine gesprochene Antwort
 * und niemanden, der wartet: Dort wird angemeldet und derselbe Titel
 * wiederholt. Genau so lief der erste erfolgreiche Kaltstart dieses Tages
 * (1122 ms, tote Nummer, PlaybackFailed, Anmeldung, PlaybackStarted).
 *
 * `MUSIK_START_LOGIN=1` holt die Anmeldung vor den ersten Ton zurueck.
 */
function startOhneAnmeldung() {
  return process.env.MUSIK_START_LOGIN !== '1';
}

/**
 * Was eine Anmeldung ohne Gegenprobe kostet.
 *
 * Die Schwelle, ab der sie vor dem ersten Ton unterblieb - inzwischen
 * unterbleibt sie dort ganz, siehe `startOhneAnmeldung`. Die Zahlen bleiben
 * als Messung stehen: Sie sagen, was eine Anmeldung im Fehlerweg kostet.
 *
 * **Der Login war der einzige Schritt, der das Eilziel nie angesehen hat.**
 * Weckruf und Nachfrage weichen ihm seit #102, die Anmeldung nicht - und sie
 * ist die teuerste von allen. Gemeldet, mit tatsaechlich toter Nummer:
 *
 ...
 *
 * Danach kein einziges AudioPlayer-Ereignis. Die Nummer in dieser Antwort war
 * frisch und richtig - sie kam nur 350 ms zu spaet, und damit war sie nichts
 * wert. Im selben Test spielte eine halbe Minute spaeter eine Antwort nach
 * 1737 ms, deren Adresse der Echo **nicht** laden konnte: Er meldete
 * `PlaybackFailed`, der Skill wiederholte den Titel, und es lief.
 *
 * Das ist die Lehre in einem Satz: **Eine tote Nummer, die rechtzeitig
 * ankommt, ist mehr wert als eine frische, die zu spaet kommt.** Die erste
 * bringt eine zweite Runde mit frischen acht Sekunden und einer warmen
 * Function; die zweite bringt Stille.
 *
 * Die Schwelle ist die gemessene Dauer einer Anmeldung ohne Gegenprobe (895,
 * 1432, 1536 ms). Sie gilt nur vor dem ersten Ton - mitten in der Wiedergabe
 * gibt es kein Eilziel, und dort ist die Anmeldung ihre Zeit wert.
 */
const LOGIN_ERWARTET_MS = 1400;
...
/**
 * Was die Nachfrage wirklich kostet - und die Schwelle, ab der sie unterbleibt.
 *
 * **Hier stand `SID_PRUEF_MS`, und damit wurde das Falsche verglichen.** Die
 * 2000 sind die Obergrenze des Abrufs, nicht seine Dauer; gemessen liegt sie
 * zwischen 565 und 946 ms. Wer bei 1999 ms Eilziel die Nachfrage fallen
 * laesst, spart also 650 ms - und landet in der Anmeldung, die 1443 bis 2635
 * kostet. Genau das ist gemeldet, in zwei Zeilen desselben Requests:
 *
 ...
 *
 * Der zweite Versuch eine halbe Minute spaeter fragte nach (647 ms), bekam
 * "gilt noch" - und **dieselbe Nummer**, die der Login gerade geholt hatte.
 * Die Sitzung hatte die ganze Zeit gelebt; die Anmeldung war reine Wartezeit,
 * und sie allein hat den Unterschied zwischen Ton und Stille gemacht.
 *
 * Die Schwelle ist deshalb die Dauer der Nachfrage, grosszuegig gerundet.
 * Darunter wird weder gefragt noch angemeldet, sondern mit der gemerkten
 * Nummer gespielt.
 */
const SID_PRUEF_ERWARTET_MS = 900;
 ...
 */
async function weckeErstenTitel(playlist, direktive, rest, restEil = () => Infinity, fritz = OHNE_FRITZ) {
  if (playlist?.quelle?.typ !== 'fritz' || !direktive) return true;
  const url = direktive.audioItem.stream.url;
  const nachLogin = fritz.angemeldet;

  // **Wo die Nummer bewusst nicht erneuert wurde, sagt der Weckruf nicht ab.**
  // Dann ist bereits entschieden, dass eine tote Nummer rechtzeitig mehr wert
  // ist als eine frische zu spaet (siehe `LOGIN_ERWARTET_MS`) - und was der
  // Weckruf hier fände, waere genau die Absage der Box, mit der diese
  // Entscheidung ohnehin rechnet. Ihn trotzdem laufen zu lassen, hiesse die
  // Eile zu bezahlen und ihren Zweck wegzuwerfen.
  if (fritz.ungeprueft) return true;
...
async function fritzAufgefrischt(playlists, body, redis, rest = () => Infinity, restEil = () => Infinity) {
  ...
  // **Wer keine Adresse herausgibt, braucht auch keine frische Nummer.**
  // Gemeldet aus einem Log, in dem eine einzige Zeile stand:
  //
  //   musik-box FRITZ!NAS-Login ok nach 1572 ms, 4355 ms Budget uebrig
  //   musik-box IntentRequest in 1603 ms
  //
  // Kein "Geraet kann", kein "spielt" - also ein Intent, der gar nichts
  // abspielt: ein missverstandener Satz, eine Rueckfrage, ein Stopp. Und
  // trotzdem eine **Anmeldung**, die laut AVM alle Sitzungen der Box beendet -
  // waehrend der Echo gerade streamte. Das ist genau der Schaden, gegen den
  // die Nachfrage vor der Anmeldung eingebaut wurde, nur auf einem Weg, den
  // bis dahin niemand angesehen hat: Die Auffrischung lief fuer **jede**
  // Anfrage, weil `gemeintePlaylist` beim fehlenden Slot auf den laufenden
  // Stream zurueckfaellt.
  //
  // Aufgefrischt wird jetzt nur noch dort, wo gleich eine Adresse an den Echo
  // geht. Alles andere - die Liste vorlesen, die Hilfe, "das habe ich nicht
  // verstanden", Stopp, die Ereignisse am Titelende - laesst die Sitzung in
  // Ruhe.
  if (!brauchtSitzung(body)) return { playlists, fritz: OHNE_FRITZ };
  ...
  // **Und eine verderbliche Adresse ueberholt sie ebenfalls - jedes Mal.**
  // Wo die Sitzungsnummer in der Adresse steckt, ist "die Frist laeuft noch"
  // eine Aussage ueber die Uhr, nicht ueber die Box: Fuenf Minuten sind der
  // Sicherheitsabstand zu AVMs zehn, und beobachtet haelt diese Box ihre
  // Sitzungen kuerzer. Der Titelwechsel, "naechster Titel", "voriger",
  // Mischen - ueberall geht gleich eine Adresse heraus, die der Echo in
  // derselben Sekunde abruft, und davor lag ein ganzer Titel, in dem die Box
  // von diesem Skill nichts gehoert hat. Die Nachfrage kostet dort rund 600
  // ms, verlaengert die Sitzung bei der Box im selben Zug und verhindert die
  // Anmeldung, die sonst beim naechsten Stolperer faellig waere.
  //
  // Ausgenommen bleibt `PlaybackNearlyFinished`: Dort geht bei einer
  // verderblichen Adresse gar nichts mehr heraus (siehe `adresseVerderblich`),
  // also gibt es auch nichts frisch zu halten.
  const gibtGleichEineAdresse = body?.request?.type !== 'AudioPlayer.PlaybackNearlyFinished';
  const erzwingen = body?.request?.type === 'AudioPlayer.PlaybackFailed'
    || faengtAn(body)
    || (adresseVerderblich(gemeint) && gibtGleichEineAdresse);
  ...
  // **Vor dem ersten Ton wird nicht angemeldet.** Siehe `ohneAnmeldung`.
  const { playlist: frisch, verlaesslich, erneuert, ungeprueft, angemeldet } = await mitFrischerSid(
    gemeint, redis, erzwingen, rest, eile,
    faengtAn(body) && startOhneAnmeldung() ? 'vor dem ersten Ton' : '',
  );
  return {
 ...
 */
async function mitFrischerSid(playlist, redis, erzwingen = false, rest = () => Infinity, restEil = () => Infinity, ohneAnmeldung = '') {
  if (playlist?.quelle?.typ !== 'fritz') return { playlist, verlaesslich: true, erneuert: false, ungeprueft: false, angemeldet: false };
  const { sid, verlaesslich, erneuert, ungeprueft, angemeldet } = await fritzSid(
    playlist.quelle.link, redis, erzwingen, rest, restEil, playlist.quelle.ordner || null, ohneAnmeldung,
  );
  // **Gar keine Nummer, und trotzdem ein Versuch - wenn die Anmeldung bewusst
  // unterblieben ist.** Hier stand, ohne Nummer gebe es nichts, worauf sich
  // ein Versuch stuetzen koenne: In der Playlist stehen dann die Adressen aus
  // der Importzeit, und die sind mit Sicherheit abgelaufen. Das stimmt - nur
  // ist der Vergleich nicht "abgelaufene Adresse gegen frische", sondern
  // "abgelaufene Adresse gegen gar keine Wiedergabe". Eine abgelaufene bringt
  // einen `PlaybackFailed`, und der repariert sich selbst; eine Anmeldung vor
  // dem ersten Ton bringt Stille (siehe `ohneAnmeldung` in `fritzSid`).
  if (!sid) return { playlist, verlaesslich: false, erneuert: false, ungeprueft, angemeldet };
  return {
 ...
 */
async function fritzSid(link, redis, erzwingen = false, rest = () => Infinity, restEil = () => Infinity, ordner = null, ohneAnmeldung = '') {
  ...
  const eiligOhneFrist = !passt && restEil() < SID_PRUEF_ERWARTET_MS;
  if (eiligOhneFrist && eigene) {
    // **Und dann wird auch nicht angemeldet.** Wer die Nachfrage der Eile
    // wegen auslaesst und danach in die Anmeldung geht, hat den billigen Weg
    // gespart, um den teuren zu nehmen - siehe `SID_PRUEF_ERWARTET_MS`.
    // Gespielt wird mit der gemerkten Nummer: Dass ihre Frist abgelaufen ist,
    // sagt etwas ueber die Uhr und nichts ueber die Box, und ein Irrtum
    // traegt sich ueber `PlaybackFailed` selbst - derselbe Weg wie bei
    // "keine Zeit zu fragen" weiter unten.
    console.warn(`musik-box Nachfrage ausgelassen und nicht angemeldet, nur noch ${restEil()} ms Eilziel`);
    return { sid: eigene, verlaesslich: false, erneuert: false, ungeprueft: true, angemeldet: false };
  }
  ...
  // **Und dasselbe gilt fuer das Eilziel.** Der Login war bisher der einzige
  // Schritt, der es nicht angesehen hat - ausgerechnet der teuerste. Siehe
  // `LOGIN_ERWARTET_MS`: Eine tote Nummer, die schnell beim Echo ankommt,
  // schlaegt eine frische, die zu spaet kommt. Die erste bringt einen
  // `PlaybackFailed` und damit eine zweite Runde mit frischen acht Sekunden;
  // die zweite bringt gar nichts, weil der Echo die Direktive dann nicht mehr
  // ausfuehrt.
  if (ohneAnmeldung) {
    console.warn(`musik-box Anmeldung ausgelassen (${ohneAnmeldung})`
      + `${eigene ? '' : ' – und es gibt keine gemerkte Nummer'}`
      + ' – es bleibt bei der Nummer, die da ist');
    return { sid: eigene, verlaesslich: false, erneuert: false, ungeprueft: true, angemeldet: false };
  }
  ...
  const begonnen = Date.now();
  // Reicht das Eilziel nicht mehr fuer eine volle Anmeldung, faellt ihre
  // zweite Haelfte weg. **Vor einem Start heisst das: immer** - selbst die
  // schnellste gemessene Anmeldung (1443 ms) laesst mit dem Vorlauf davor
  // keinen weiteren Weg zur Box mehr uebrig. Ohne Eile
  // (`MUSIK_EILZIEL_MS=0`) ist restEil unendlich, und es bleibt bei der
  // Gegenprobe; mitten in der Wiedergabe ebenso, denn dorthin reicht
  // `fritzAufgefrischt` gar kein Eilziel.
  const knapp = restEil() < LOGIN_VOLL_MS;
  const ergebnis = await frischeSid(freigabe, rest() - SID_ANTWORT_RESERVE_MS, knapp);
...
/**
 * Die Sitzungen warm halten - angestossen von aussen, ohne Alexa.
 *
 * **Warum das der Skill nicht selbst kann.** Eine FRITZ!NAS-Sitzung stirbt
 * nach etwa zehn Minuten Ruhe, und der erste Start danach zahlt den Preis:
 * Gemessen am 20. September brauchte der Echo drei bis zwoelf Sekunden, bis
 * er die tote Adresse aufgab, bevor der Fehlerweg sie ersetzen konnte. Vom
 * Befehl bis zum Ton waren es neun beziehungsweise einundzwanzig Sekunden -
 * es spielte, aber es liess auf sich warten. Wer die Sitzung nie ablaufen
 * laesst, hat diesen Umweg nicht: Die Box verlaengert sie bei **jedem**
 * Zugriff, und `check_nas_rights` ist der billigste, den es gibt.
 *
 * **Und die Platte bleibt dabei in Ruhe.** Das war der Grund, aus dem dieser
 * Endpunkt im September wieder ausgebaut wurde - die Sorge, ein Takt von
 * aussen halte die Platte am NAS rund um die Uhr wach. Er fasst aber keine
 * Datei an: `check_nas_rights` beantwortet die Box aus ihrem
 * Sitzungsspeicher. Was die Platte weckt, ist der Weckruf vor dem ersten Ton
 * (`weckeStream`), und der laeuft nur, wenn wirklich jemand Musik hoert.
 *
 * **Erst fragen, dann hoechstens einmal anmelden - und nur, wenn gar nichts
 * mehr lebt.** Eine Anmeldung beendet alle Sitzungen der Box. Solange noch
 * eine lebt, waere sie ein Tausch und kein Gewinn, und beim naechsten Lauf
 * ginge dasselbe Spiel mit der naechsten Freigabe von vorn - erst A, fuenf
 * Minuten spaeter B und damit A wieder. Ist dagegen alles tot, gibt es nichts
 * zu verlieren; angemeldet wird die zuletzt benutzte Freigabe, weil sie die
 * wahrscheinlichste fuer den naechsten Start ist, und die Nachfrage haelt sie
 * danach am Leben.
 *
 * **Mehr als eine warme Sitzung ist nicht zu haben.** `filelink.lua` wird
 * ohne Sitzung geoeffnet, und das ist laut AVM genau der Zugriff, der alle
 * bestehenden beendet. Bei sechs Ordner-Playlists heisst das: eine ist warm,
 * die uebrigen holen sich ihre Nummer beim Spielen ueber den Fehlerweg. Der
 * erste Start einer *anderen* Playlist geht damit weiter ueber den Umweg -
 * nur der auf dieselbe nicht mehr, und das ist der haeufige Fall.
 *
 * **Abschalten:** `MUSIK_WARM=0` in der Vercel-Umgebung. Der Endpunkt
 * antwortet dann weiter (der Takt draussen muss nichts wissen), fasst die Box
 * aber nicht mehr an.
 *
 * **Ohne Alexas Fenster, aber nicht ohne Frist:** Die Function selbst hat
 * zehn Sekunden, und bei mehreren Freigaben teilen sie sich diese.
 */
export async function handleWarm(req, res, redis) {
  // POST, weil der Aufruf etwas tut: Er fasst die Box an und kann im Notfall
  // anmelden. Ein GET, den ein Browser-Prefetch beilaeufig ausloest, waere
  // dafuer die falsche Tuer.
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  if (process.env.MUSIK_WARM === '0') {
    console.log('musik-box Sitzungen warm halten: abgeschaltet (MUSIK_WARM=0)');
    return res.status(200).json({ ok: true, warm: false, grund: 'abgeschaltet (MUSIK_WARM=0)' });
  }

  const playlists = await redis.get(REDIS_KEY) || [];
  // Link -> Ordner, damit die Nachfrage die Nummer auch gegen die richtige
  // Freigabe pruefen kann (siehe `fremderOrdner`).
  const freigaben = new Map();
  for (const p of playlists) {
    if (p.quelle?.typ === 'fritz' && p.quelle.link && !freigaben.has(p.quelle.link)) {
      freigaben.set(p.quelle.link, p.quelle.ordner || null);
    }
  }
  if (freigaben.size === 0) {
    console.log('musik-box Sitzungen warm halten ausgelassen: keine FRITZ!NAS-Playlist');
    return res.status(200).json({ ok: true, warm: false, grund: 'keine FRITZ!NAS-Playlist' });
  }

  // **Erst fragen, dann hoechstens einmal anmelden.** Die Reihenfolge nach
  // dem Zeitstempel: Die zuletzt benutzte Freigabe ist die wahrscheinlichste
  // fuer den naechsten Start, und wenn nur eine Sitzung zu haben ist, soll es
  // diese sein.
  const tafel = await fritzSidGemerkt(redis);
  const reihe = [...freigaben.keys()].sort((a, b) => (tafel[b]?.zeit || 0) - (tafel[a]?.zeit || 0));

  const frist = Date.now() + WARM_GESAMT_MS;
  const rest = () => Math.max(0, frist - Date.now());
  const ergebnisse = [];
  const tot = [];
  for (const link of reihe) {
    if (rest() < WARM_MINDEST_MS) {
      ergebnisse.push({ link: freigabeKurz(link), warm: false, grund: 'keine Zeit mehr' });
      continue;
    }
    const begonnen = Date.now();
    const { sid, verlaesslich } = await fritzSid(
      link, redis, true, rest, () => Infinity, freigaben.get(link),
      'erst wird gefragt, angemeldet wird hoechstens einmal',
    );
    if (!verlaesslich) tot.push(link);
    ergebnisse.push({
      quelle: link, link: freigabeKurz(link), warm: verlaesslich, ms: Date.now() - begonnen,
      sid: verlaesslich && sid ? `…${sid.slice(-4)}` : null,
    });
  }

  // **Angemeldet wird nur, wenn gar nichts mehr lebt.** Eine Anmeldung beendet
  // alle Sitzungen der Box - solange noch eine lebt, waere sie ein Tausch und
  // kein Gewinn, und beim naechsten Lauf ginge dasselbe Spiel mit der
  // naechsten Freigabe von vorn. Ist dagegen alles tot, gibt es nichts zu
  // verlieren, und danach haelt die Nachfrage diese eine Sitzung am Leben.
  //
  // **Mehr als eine ist nicht zu haben**, solange `filelink.lua` ohne
  // Sitzung geoeffnet wird - das ist laut AVM genau der Zugriff, der alle
  // bestehenden beendet. Bei sechs Ordner-Playlists heisst das: eine ist warm,
  // fuenf holen sich ihre Nummer beim Spielen ueber den Fehlerweg.
  const alleTot = tot.length === ergebnisse.length && tot.length > 0;
  if (alleTot && rest() >= WARM_MINDEST_MS) {
    const link = tot[0];
    const begonnen = Date.now();
    const { sid, verlaesslich } = await fritzSid(link, redis, true, rest, () => Infinity, freigaben.get(link));
    // Ueber den ganzen Link, nicht ueber die gekuerzte Anzeige: Zwei
    // Freigaben koennen auf dieselben vier Ziffern enden.
    const eintrag = ergebnisse.find(e => e.quelle === link);
    if (eintrag) {
      eintrag.warm = verlaesslich;
      eintrag.angemeldet = true;
      eintrag.ms += Date.now() - begonnen;
      eintrag.sid = verlaesslich && sid ? `…${sid.slice(-4)}` : null;
    }
    console.log(`musik-box keine Sitzung lebte mehr – fuer ${freigabeKurz(link)} neu angemeldet:`
      + ` ${verlaesslich ? 'ok' : 'gescheitert'}`);
  }

  const warm = ergebnisse.filter(e => e.warm).length;
  console.log(`musik-box Sitzungen warm gehalten: ${warm} von ${freigaben.size}`
    + `${freigaben.size > 1 ? ' (mehr als eine ist nicht zu haben – jede Anmeldung beendet die anderen)' : ''}`
    + ` (${ergebnisse.map(e => `${e.link}: ${e.warm ? 'ja' : 'nein'}${e.angemeldet ? ', neu angemeldet' : ''}`).join('; ')})`);
  return res.status(200).json({
    ok: warm > 0,
    warm,
    gesamt: freigaben.size,
    freigaben: ergebnisse.map(({ quelle, ...rest }) => rest),
  });
}

/** Die Freigabenummer fuer das Log - der ganze Link ist der Schluessel zum Ordner. */
function freigabeKurz(link) {
  const id = String(link).match(/[?&]id=([^&]+)/)?.[1] || '';
  return id ? `…${id.slice(-4)}` : 'ohne id';
}

/** Was der ganze Warm-Lauf hoechstens brauchen darf. Die Function hat zehn
 *  Sekunden; hier wartet niemand, aber ein Timeout ist auch keine Antwort. */
const WARM_GESAMT_MS = 8000;
/** Darunter lohnt fuer eine weitere Freigabe kein Anlauf mehr. */
const WARM_MINDEST_MS = 1500;
 ...
 *
 *   stumm:   2539, 2857, 3271, 3683, 3846, 4537, 5204, 5771 ms
 *   spielt:  1519, 1569, 1737, 1868, 2039, 2447, 2524 ms
 *
 ...
 *
 * **Die beiden mittleren Punkte sind aus demselben Test.** 2539 und 2857 auf
 * der stummen Seite, 1569 und 1737 auf der spielenden - die Luecke liegt
 * damit zwischen 2524 und 2539, und die zweieinhalb Sekunden unten sind keine
 * runde Zahl mehr, sondern gemessen. Der Punkt bei 1737 ist der lehrreichste:
 * Dort konnte der Echo die Adresse **nicht** laden, meldete `PlaybackFailed`,
 * und der wiederholte Titel lief. Rechtzeitig und falsch schlaegt zu spaet
 * und richtig.
 *
 ...
 */
/**
 * Ereignisse, die eine Stream-Adresse herausgeben, ohne ein Intent zu sein:
 * das Anhaengen am Titelende, der Wiederholversuch nach einem Fehlschlag und
 * die Knoepfe der Alexa-App.
 */
const STREAM_EREIGNISSE = new Set([
  'AudioPlayer.PlaybackNearlyFinished',
  // Seit der naechste Titel einer FRITZ!NAS-Playlist erst hier bestellt wird
  // (siehe `adresseVerderblich`), geht auch von hier eine Adresse heraus.
...
/**
 * Gibt diese Anfrage gleich eine Adresse an den Echo heraus?
 *
 * Nur dann lohnt eine frische Sitzungsnummer - und nur dann ist der Preis
 * gerechtfertigt, den eine Anmeldung im schlimmsten Fall kostet: alle
 * Sitzungen der Box, den gerade laufenden Titel eingeschlossen.
 *
 * Bei den Intents ist das genau dieselbe Liste, die auch entscheidet, ob ein
 * Geraet ohne AudioPlayer einen Satz statt eines Versprechens bekommt - beide
 * Fragen meinen dasselbe: "Kommt hier gleich Musik?"
 */
function brauchtSitzung(body) {
  const typ = body?.request?.type;
  if (typ === 'IntentRequest') return WIEDERGABE_INTENTS.has(body.request.intent?.name);
  return STREAM_EREIGNISSE.has(typ);
}
...
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity, restEil = () => Infinity, fritz = OHNE_FRITZ) {
  ...
  if (playlist.quelle?.typ === 'fritz' && !fritz.ok) {
    ...
    console.warn(`FRITZ!NAS-Sitzung nicht erneuert (keine Zeit dafuer),`
  ...
  if (!await weckeErstenTitel(playlist, spiel, rest, restEil, fritz)) {
 ...
 */
async function losOderAbsage(body, res, playlist, start, direktiven = [], rest = () => Infinity, restEil = () => Infinity, fritz = OHNE_FRITZ) {
  if (await weckeErstenTitel(playlist, start, rest, restEil, fritz)) return still(res, [...vorspann(), start], ausSprache(body));
...
async function weiter(body, res, playlists, direktiven = [], stumm = false, redis = null, rest = () => Infinity, restEil = () => Infinity, fritz = OHNE_FRITZ) {
  ...
  if (lage) {
    ...
    return await losOderAbsage(body, res, lage.playlist, start, direktiven, rest, restEil, fritz);
  }
  ...
  if (juengste) {
    ...
    return await losOderAbsage(body, res, juengste.playlist, start, direktiven, rest, restEil, fritz);
  }
...
function naechsterTitel(body, res, playlists) {
  ...
  // **Bei FRITZ!NAS wird hier nichts angehaengt.** Die Adresse traegt eine
  // Sitzungsnummer, und dieses Ereignis kommt Sekunden nach dem Titelanfang -
  // der Echo spielt den angehaengten Titel aber erst vier Minuten spaeter.
  // Siehe `adresseVerderblich`.
  if (adresseVerderblich(lage.playlist)) return still(res);
...
/**
 * Verdirbt die Adresse dieser Playlist, bevor der Echo sie benutzt?
 *
 * **Der Fehler, der hier steckte, ist der aelteste von allen.** Eine
 * FRITZ!NAS-Adresse traegt ihre Sitzungsnummer **in sich**, und die Box
 * vergisst eine Sitzung nach Minuten. Der Skill hat sie deshalb vor jeder
 * Antwort aufgefrischt - nur ging eine dieser Antworten an eine Stelle, an
 * der "jetzt" und "wenn der Echo sie benutzt" nicht dasselbe sind:
 *
 * `AudioPlayer.PlaybackNearlyFinished` heisst nicht "kurz vor Schluss". Der
 * Echo fragt den naechsten Titel an, sobald seine Warteschlange Platz hat -
 * in der Regel **Sekunden nach dem Titelanfang**, wie es im Kommentar dort
 * seit jeher steht. Die Adresse, die er dabei bekommt, legt er in die
 * Warteschlange und ruft sie ab, wenn der laufende Titel zu Ende ist: bei
 * einem Album vier, bei einem Hoerspielkapitel zwanzig Minuten spaeter. Die
 * Nummer darin war zu diesem Zeitpunkt frisch geholt - und ist es dann seit
 * vier Minuten nicht mehr. Solange der Echo waehrenddessen laedt, haelt sein
 * eigener Abruf die Sitzung am Leben; hat er den Titel fertig gepuffert,
 * geschieht bis zum Titelwechsel gar nichts mehr, und die Sitzung stirbt an
 * genau der Stelle, an der die naechste Adresse gebraucht wird.
 *
 * Deshalb wird bei einer FRITZ!NAS-Playlist **nicht** vorgesorgt: Der
 * naechste Titel wird erst bei `PlaybackFinished` bestellt, wenn der Echo ihn
 * auch wirklich gleich abruft. Der Preis ist eine Luecke von ein, zwei
 * Sekunden zwischen den Titeln - der Preis dafuer war Stille.
 *
 * `MUSIK_FRITZ_NAHTLOS=1` holt das Anhaengen zurueck, fuer den Fall, dass
 * eine Box ihre Sitzungen laenger haelt als diese hier.
 */
function adresseVerderblich(playlist) {
  if (playlist?.quelle?.typ !== 'fritz') return false;
  return process.env.MUSIK_FRITZ_NAHTLOS !== '1';
}
...
async function titelZuEnde(body, res, playlists, redis, rest = () => Infinity) {
  ...
  // **Hier wird der naechste Titel bestellt, wenn seine Adresse verdirbt.**
  // Das Anhaengen bei `PlaybackNearlyFinished` entfaellt dann (siehe
  // `adresseVerderblich`), und die Nummer in dieser Adresse ist wenige
  // hundert Millisekunden alt, wenn der Echo sie abruft - statt so alt wie
  // der Titel, der gerade gespielt hat.
  const direktiven = [];
  if (gehtWeiter && adresseVerderblich(lage.playlist)) {
    const weiterMit = playDirektive(lage.playlist, ziel.position, ziel.runde, { seed: ziel.seed });
    // Dieselbe halbe Sekunde wie nach einem Fehlschlag: Hat der Echo den
    // letzten Titel fertig gepuffert, hat die Box seit Minuten nichts zu tun
    // gehabt, und ihre Platte steht.
    await atemHolen(lage.playlist, weiterMit, rest);
    direktiven.push(weiterMit);
  }
