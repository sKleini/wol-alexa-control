...
import {
  ...
  verlaufSchreiben, verlaufLesen, tonLaeuft,
} from './naston.js'
...
export async function handleManage(req, res, redis) {
  ...
  if (req.method === 'GET') {
    ...
    if (query.pruefen) {
      ...
      // **Nicht pruefen, waehrend gespielt wird.** Gemessen im Betrieb: Acht
      // Leseproben im Sekundentakt haben die Box verstopft - die neunte
      // brauchte fuer 32 KB dreieinhalb Minuten, und als danach der naechste
      // Titel gebraucht wurde, war sie immer noch dicht. Der Knopf hat die
      // Wiedergabe beendet, die er pruefen sollte. Der Merkzettel
      // `musik_ton_laeuft` weiss, ob gerade geliefert wird.
      if (playlist?.quelle?.typ === 'fritz' && await tonLaeuft(redis)) {
        return res.status(200).json({
          name: playlist.name, ab, gesamt: playlist.titel.length, ergebnisse: [], weiter: null,
          gesperrt: 'Playing right now – the box only answers one fetch at a time.'
            + ' Checking now would stall the playlist. Try again once playback has stopped.',
        });
      }
...
/**
 * Und eine Pause dazwischen, weil "nacheinander" nicht genug war.
 *
 * Gemessen: acht Leseproben in sechs Sekunden, jede fuer sich brav einzeln -
 * und die neunte bekam ihre 32 KB erst dreieinhalb Minuten spaeter. Die Box
 * braucht nicht nur Alleinsein, sondern auch Luft zwischen den Abrufen.
 */
const FRITZ_PRUEF_PAUSE_MS = 500;
...
async function pruefePlaylist(playlist, ab) {
  ...
  for (let i = 0; i < teil.length; i += gleichzeitig) {
    if (fritz && i > 0) await new Promise(a => setTimeout(a, FRITZ_PRUEF_PAUSE_MS));
...
export async function handleSkill(body, res, redis) {
  ...
  try {
    ...
    if (typ === 'IntentRequest') {
      ...
      switch (intent.name) {
        ...
        case 'SuchePlaylistIntent':
          return await handlePlay(intent, res, playlists, direktiven, redis, rest);
    ...
    // **Was der Echo meldet, gehoert neben das, was der Durchleiter geliefert
    // hat.** Erst beides nebeneinander beantwortet die Frage nach einem
    // Abbruch: Ein `PlaybackStopped` direkt hinter einem unvollstaendigen Ton
    // ist ein abgerissener Strom; eines ohne solchen Eintrag kam vom Echo
    // selbst. Ein Zeitstempel je Ereignis genuegt dafuer - der Verlauf ist
    // ein Zeuge, kein Protokoll.
    if (typ.startsWith('AudioPlayer.')) {
      await verlaufSchreiben(redis, {
        was: 'echo',
        ereignis: typ.slice('AudioPlayer.'.length),
        offset: body.context?.AudioPlayer?.offsetInMilliseconds ?? null,
        titel: titelZumToken(laufendes(body, playlists)) || null,
        fehler: body.request?.error?.type || null,
      });
    }
 ...
 */
async function handlePlay(intent, res, playlists, direktiven, redis, rest = () => Infinity) {
  if (playlists.length === 0) {
    return speak(res, 'Es ist noch keine Playlist angelegt. Bitte lege im Dashboard eine an.', true);
  }
...
function naechsterTitel(body, res, playlists) {
  ...
  // **Bei FRITZ!NAS wird hier nichts angehaengt** - siehe
  // `boxVertraegtKeineZwei`. Der naechste Titel wird erst bestellt, wenn
  // dieser zu Ende ist.
  if (boxVertraegtKeineZwei(lage.playlist)) return still(res);
...
/**
 * Vertraegt diese Quelle zwei Abrufe gleichzeitig? Bei einer FRITZ!Box: nein.
 *
 * **Der Grund hat gewechselt, die Antwort nicht.** Frueher stand hier, dass
 * eine Adresse mit Sitzungsnummer verdirbt, bis der Echo sie benutzt - das
 * ist mit dem Durchleiter erledigt, und deshalb ist diese Regel in #116
 * gefallen. Der zweite Grund war schon immer da und steht seit Monaten
 * zwanzig Zeilen weiter oben bei der URL-Pruefung: **Eine FRITZ!Box ist kein
 * Webspace.** Sie liefert jeden Titel ueber ein Lua-Skript aus ihrem eigenen
 * Speicher und ueber die Uplink-Leitung eines Haushalts; mehrere
 * gleichzeitige Abrufe bringen sie dazu, keinen davon rechtzeitig zu
 * beantworten.
 *
 * `AudioPlayer.PlaybackNearlyFinished` kommt Sekunden **nach** dem
 * Titelanfang. Wer dort anhaengt, schickt den Echo los, den naechsten Titel
 * zu laden, waehrend der laufende noch streamt - zwei Abrufe an derselben
 * Box, und beide gehen durch je eine Function, die nach 60 Sekunden stirbt.
 * Genau das stand im gemeldeten Log:
 *
 *   07:30:24  PlaybackNearlyFinished → ENQUEUE von Titel 3
 *   07:30:24  GET …003…   200, 16853 ms
 *   07:30:28  GET …002…   200, 60000 ms   ← Vercel Runtime Timeout
 *   07:30:29  PlaybackStopped             ← sechs Sekunden nach dem Start
 *
 * Deshalb wird bei einer FRITZ!NAS-Playlist nicht vorgereiht: Der naechste
 * Titel wird bei `PlaybackFinished` bestellt, wenn der Echo ihn auch wirklich
 * gleich abruft und der vorige nichts mehr zieht. Der Preis ist eine Luecke
 * von ein, zwei Sekunden zwischen den Titeln; der Preis dafuer war ein
 * abgebrochenes Kapitel.
 *
 * `MUSIK_FRITZ_NAHTLOS=1` holt das Anhaengen zurueck - fuer eine Box mit
 * mehr Luft oder eine Leitung, die zwei Stroeme traegt.
 */
function boxVertraegtKeineZwei(playlist) {
...
async function titelZuEnde(body, res, playlists, redis, rest = () => Infinity) {
  ...
  // **Hier wird der naechste Titel bestellt, wenn nicht vorgereiht wurde.**
  // Das ist der Moment, in dem der vorige Abruf zu Ende ist - die Box hat
  // dann wieder nur einen Strom zu bedienen. Siehe `boxVertraegtKeineZwei`.
  const direktiven = [];
  if (gehtWeiter && boxVertraegtKeineZwei(lage.playlist)) {
    direktiven.push(playDirektive(lage.playlist, ziel.position, ziel.runde, { seed: ziel.seed }));
  }
