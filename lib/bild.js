// lib/bild.js – Zwischenlager für das Bild einer „Meldung anzeigen"
//
//   POST /api/ring?bild=1          Rumpf = JPEG als base64 (JSON {"b64":"..."}
//                                  oder roher Text). Antwort: {"id":"<16 Hex>"}
//   GET  /api/ring?bild=1&id=<id>  liefert die Bytes als image/jpeg zurück
//
...
//
// **Warum in lib/ und nicht als eigenes api/bild.js.** Genau so war es zuerst
// gebaut, und genau daran scheiterte der Deploy: Vercel macht aus jeder Datei
// unter `api/` eine eigene Serverless Function, und der Hobby-Tarif erlaubt
// zwölf davon. Mit `api/bild.js` waren es dreizehn — der Build ging rot, ohne
// dass an der Sache selbst etwas falsch gewesen wäre. Dateien unter `lib/`
// zählen nicht mit; sie werden in die Funktion hineingebündelt, die sie
// importiert. Das Bild hängt ohnehin am Befehl, also gehört es zu `/api/ring`.
//
...
// nuetzt niemandem. Geprueft wird er in api/ring.js, bevor hierher verzweigt
// wird; diese Datei setzt ihn voraus.
...
/**
 * Die Verzweigung: Traegt die Anfrage `bild`, geht sie hierher.
 *
 * Ein eigener Parameter und nicht etwa die Methode als Unterscheidung: `POST`
 * benutzt `/api/ring` bereits fuer gewoehnliche Befehle, und eine Weiche, die
 * am Verb haengt, waere beim naechsten Blick nicht mehr zu erkennen.
 *
 * Rein und exportiert, damit die Weiche selbst pruefbar bleibt.
 */
export function istBildAnfrage(query) {
  return query != null && query.bild !== undefined;
}

/**
 * Behandelt die Bild-Anfrage. Der Schluessel ist zu diesem Zeitpunkt geprueft.
 */
export async function bildHandler(req, res) {
  if (req.method === 'POST') {
