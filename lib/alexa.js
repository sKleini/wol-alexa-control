 ...
 *
 * **Bleibt die Sitzung offen, gehoert ein `reprompt` dazu.** Ohne eines
 * beendet Alexa sie nach wenigen Sekunden *kommentarlos*: Das Mikrofon geht
 * zu, und der naechste Satz erreicht nicht mehr den Skill, sondern Alexa
 * allgemein. Fuer den Sprechenden sieht das aus, als habe der Skill nicht
 * zugehoert - er wiederholt sich lauter, und beim zweiten Mal, schneller
 * gesprochen, klappt es. Genau dieses "geht nur manchmal beim ersten Versuch"
 * hatte die Musik Box, und es stand an keiner Stelle im Code, weil es gar
 * keine gab: `reprompt` kam im ganzen Repository nicht vor.
 *
 * Ohne eigenen Text wird der gesprochene Satz wiederholt. Das ist immer
 * richtig und nie falsch; wo das Nachfragen kuerzer sein soll als die erste
 * Frage, reicht der Aufrufer einen eigenen Text herein.
 *
 * Bei einer Antwort, die die Sitzung beendet, waere ein reprompt ungueltig -
 * deshalb steht es hinter der Bedingung und nicht daneben.
 */
export function speak(res, text, endSession = true, direktiven = [], reprompt = null) {
  const response = {
    outputSpeech: { type: 'PlainText', text },
    shouldEndSession: endSession,
  };
  if (!endSession) {
    response.reprompt = { outputSpeech: { type: 'PlainText', text: reprompt || text } };
  }
