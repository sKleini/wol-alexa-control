...
- **Resume** picks up where the playlist last stopped, and it has **two gaits**, because a record and an audiobook want different things. It works across days and across Echo devices, and the position is kept per playlist, not per person: whoever carries on in the kids' room continues where the living room left off, which is what a household wants and what a public skill would call a flaw. A finished playlist and *"von vorn"* both clear the mark.
  ...
  - **What the history draws, it draws by name.** Each line is a `WORT` (a spoken command), an `ECHO` (a player event) or a `TON` (a fetch from the box), and anything it cannot place appears as `????` with its raw content rather than being forced into one of them. `TON` lines carry the requested range — `bytes=0-` is the whole file, `bytes=4187-` the jump over the ID3 header, anything beyond that a real seek into the middle — and `ECHO` lines now carry the gait as well, which they had been storing all along without showing it.
...
| The history shows `TON   undefined  undefined B in undefined ms`, and the `WORT` lines are missing | **the open page was older than the server.** *Refresh* in the log box fetches the data, never the page, so a tab opened before a deploy keeps running its old renderer — and that one did not know the `wort` entries. They fell into the `ton` branch, which was the silent `else`, and were drawn as a fetch that never happened | fixed twice over: the `ton` branch is now recognised by name and an unknown entry shows itself (`????` plus its raw content) instead of being mistaken for one, and every `/api/manage` answer carries `x-app-version` so the page says *"Dashboard veraltet – Seite neu laden"* when the server moves on beneath it |
| The history shows a `WORT` line but no `Stand:` beside it | same cause — `Stand:` also only exists in the newer renderer | reload the page; if it stays away afterwards, the write really did not run and the Vercel log says so |
