...
| Start a playlist, stop shortly after, start again — and it stays silent | the Echo was paused on exactly that stream and got **the same address** again. The skill answers correctly and in milliseconds, the Echo fetches the file and receives all of it — and does not play it. A different stream token alone was not enough (#137) | every directive now carries its own address (`&n=…` behind the signed token; the endpoint reads only `ton`). The start line in the log names the token handed out, the one the device still holds and its `playerActivity`, so the next occurrence is decidable from the log alone. If it still happens, try `MUSIK_CLEAR_QUEUE=0` |
