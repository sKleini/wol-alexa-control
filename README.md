...
- **Secure SHA-256 Topics**: Commands travel over an ntfy.sh topic whose name is derived by SHA-256 from the PC's MAC address and your private `ADMIN_PASSWORD`, so it cannot be guessed.
...
| `FCM_PROJECT_ID`, `FCM_CLIENT_EMAIL`, `FCM_PRIVATE_KEY` | *(optional, location feature)* Firebase service account used by `/api/ring` (and the zone-change pushes) to send pushes to the Mylo app and the Actions Hub. Without all three, `/api/ring` answers `{ok:false, reason:"fcm_not_configured"}` |
...
| `MUSIK_VORLAUF_MS` | *(optional, Meine Plattenkiste)* How far *Resume* rewinds behind the remembered spot, in milliseconds, in its **Audiobook** setting. Default `10000`; `0` resumes on the exact millisecond. It is subtracted when *starting*, never when *remembering* — the database holds the raw position — and it applies to whichever of the two sources won. No effect on **Album**, which always restarts the track. It is also the threshold below which an incoming position counts as "the start of the track" rather than as a position, and therefore may not overwrite a remembered second (see **Resume**). Re-read on every request, like `MUSIK_BUDGET_MS` |
| `MUSIK_BUDGET_MS` | *(optional, Meine Plattenkiste)* Time budget of one skill request in milliseconds. Default `6500` (Alexa's eight seconds minus a 1.5 s reserve); the time Alexa has already waited is subtracted, with a floor of 1200 ms. Re-read on every request (see **Why the skill never goes silent**) |
...
| `MUSIK_TON_STILLSTAND_MS` | *(optional, Meine Plattenkiste)* After how many milliseconds without a single byte being taken a delivery is ended. Default `60000`; `0` switches the watchdog off. A paused Echo does not close the connection, it only stops reading, so `close` never arrives and the delivery would otherwise run on to the function's clock — on the very instance that has to answer Alexa within eight seconds. It ends a delivery that has **stopped**, not one that is merely slow: while bytes are still being taken someone is listening |
| `MUSIK_STARTWACHE` | *(optional, Meine Plattenkiste)* How many milliseconds a start has to report back before it counts as silent; `0` switches the watch off. Default `15000` — in the logs `PlaybackStarted` arrived 1 to 5 seconds after the fetch. A `Play REPLACE_ALL` the Echo drops leaves no trace of its own: the skill answers in milliseconds, the Echo fetches the file and receives all of it, and then nothing comes — no `PlaybackStarted`, no `PlaybackFailed`, no `ExceptionEncountered`. The watch turns that into a dated entry in the history, with both stream tokens beside it |
| `MUSIK_TON_LAUF` | *(optional, Meine Plattenkiste)* `0` switches off the cancelling of superseded deliveries. Every start address carries `&g=1` and calls out a new playback; an appended track (`ENQUEUE`) joins the one that stands, so a track playing while the next is prefetched is never cut. When a newer playback starts, the older deliveries stop within five seconds. Measured before this: 180 MB in two and a half minutes for six files, 49 MB of it for two tracks that were never heard. The counter is one for the whole app, not one per device — two Echos playing different playlists at the same time would cancel each other, which is what `0` is for |
| `MUSIK_CLEAR_QUEUE` | *(optional, Meine Plattenkiste)* `0` drops the `AudioPlayer.ClearQueue` (`CLEAR_ALL`) that otherwise precedes every start. It is the long-standing suspicion against a device that sits silent on old state — and `CLEAR_ALL` also stops whatever is playing, so if a start ever goes silent on a paused device, this is the first thing to try. No deploy needed |
...
- Check the **MAC Address** of the PC — pre-filled with the address the agent detects; it must match the dashboard entry.
...
3. Open the **JSON Editor** under *Interaction Model* and paste the contents of
   ...
   value under `PERSON_NAME` per person**. The skill also pushes the persons from the dashboard as
   dynamic slot values with every answer, but those only take effect *after* an answer and expire
   after a while; a name found in neither place makes the skill answer
   *"Ich habe keine Person namens … gefunden."* (details in [`alexa/README.md`](alexa/README.md)).
...
**How it works:** the dashboard stores playlists in Redis (`musik_playlists`, plus `musik_stand` for the resume marks). A third Alexa skill (type **Custom**, with the **AudioPlayer** interface) reads them and answers every `PlayPlaylistIntent` with an `AudioPlayer.Play` directive. As soon as her queue has room Alexa asks the skill for the next track (`PlaybackNearlyFinished`) and gets it enqueued — after the last one, the first again. The state lives in the stream token (`<playlist>|<position>|<round>|<seed>[|<attempt>[|<streak>]]`), not in the database, so nothing can go stale — the one exception is the resume mark, which is the one thing a token cannot carry across a silent night.
...
**While bytes are flowing, the box is never asked to log in — without exception.** A login ends *every* session on the box, including the one streaming the chapter you are hearing; that is the one possible cause of a mid-chapter stop that this code itself creates. The guard was introduced half-done (`laeuft && !erzwingen`): the forced second attempt still logged in, and that attempt is exactly the one that runs when the box rejects the remembered number — so the half guard was none. Now the switch holds for both attempts. It also holds across a track change, where two deliveries overlap for a few seconds: each one has its own entry (`musik_ton_laeufer`, a sorted set with one expiry per delivery, refreshed every minute), so the one that finishes first no longer takes the other's protection with it. The price is stated plainly: if the box rejects while a delivery is running, *that* track fails. One track instead of an evening.
...
- **Resume** picks up where the playlist last stopped, and it has **two gaits**, because a record and an audiobook want different things. It works across days and across Echo devices, and the position is kept per playlist, not per person: whoever carries on in the kids' room continues where the living room left off, which is what a household wants and what a public skill would call a flaw. A finished playlist and *"von vorn"* both clear the mark.
  ...
  - **Audiobook** remembers the *second*. Stop thirty seconds into a three-minute chapter and the next start resumes at 0:20 — ten seconds behind the mark (`MUSIK_VORLAUF_MS`), because whoever stops in mid-sentence wants the sentence, not its second half. That rewind settles the other edge too: stop after nine seconds and the track starts over instead of skipping them.
  ...
  - **The mark is written before the answer, not after it.** For one release it was written after — in the handler's `finally`, on the reasoning that Alexa is no longer waiting there. That is true of Alexa and false of Vercel: the invocation ends with the response, the instance freezes, and whatever is still pending resumes only when that same instance is called again. The logs caught it exactly: a `PlaybackStopped` and an `AMAZON.PauseIntent` answered at 18:45:34 had their `finally` blocks run at **18:50:01**, attached to an unrelated request — 4 minutes 27 seconds later, with a deadline that had expired during the freeze. Both writes failed, a third never ran at all because the instance was recycled first, and of three remembered seconds not one reached the database. So the write happens where it can actually happen: before the response. A player event has nothing to answer and gets a floor of its own (2000 ms) rather than the leftover response budget; a spoken stop keeps to the budget, because a directive hangs on its answer. The price is one database round trip before the silence — in the reported case there were 6463 ms of headroom and 10 ms were used.
  - **A failed write says so.** `standSchreiben` used to give up silently on an expired deadline while the caller reported "remembered" anyway, so the Vercel log and the dashboard history both showed `113130 ms gemerkt (stopp)` on a line where nothing had been stored. Whoever went looking for the lost second read a success message at precisely the wrong place. It now reads `NICHT gespeichert`.
  - **The history entry is still written after the answer**, and that one is a deliberate trade rather than a promise: it carries what went out, which is only known once the answer is out. The same freeze applies to it — in the reported case a stop and its line in the dashboard were 4 minutes 27 seconds apart, and in between the log looked as if nothing had happened. Acceptable for a piece of information; it was not acceptable for the second itself.
  - **Nothing below the pre-roll overwrites a remembered second** of the same track. A track start says nothing about the tenth minute of that same track still being the place to go back to — and that is what broke *Audiobook*: a resume that once landed at the beginning erased, with its own `PlaybackStarted`, the mark that would have saved it next time. After that no attempt had anything better to read, and a single misstep turned into a permanent *"always from the beginning"*.
    - **The rule used to say "a zero", and that was too narrow.** The reported case had `@1093 ms` in the database — not a zero, so it went through and buried the real mark. The number itself is honest: the dashboard history shows a whole chain of playbacks that ended after 8, 11, 21 and 42 seconds, on tracks about an hour long. If playback dies after a second, the last known position *is* a second. It is just worthless as a destination — `einstieg()` turns it into 0 — and it must not erase the tenth minute that stood there before. An attempt that dies immediately says nothing about the evening. The threshold is the pre-roll itself, with no arbitrariness left in it: `MUSIK_VORLAUF_MS` is subtracted anyway, so anything below it ends up at 0. What the arithmetic turns into "the start of the track" may not erase a real position. At `MUSIK_VORLAUF_MS=0` it is the exact zero again — there every millisecond is a position, and that is what the setting is for.
    - **It does not cover a stop above the pre-roll.** A playback that dies after 11 or 21 seconds writes a real position, and that position replaces a larger one. Whether the mark should never move backwards inside a track is a separate question — the pre-roll shaves ten seconds off every legitimate resume, so a blanket rule is not free — and the honest answer is to stop the playback from dying rather than to teach the mark to ignore it.
    - The price, named rather than hidden: stop 2 seconds into a track whose tenth minute is remembered, and the mark stays at 10:00. Those two seconds would have been the start of the track after the pre-roll anyway, and *"von vorn"* clears a mark on purpose.
    - **Above the pre-roll every value is written**, including a smaller one: the pre-roll shaves ten seconds off with every resume, and whoever stops at 0:30 after jumping back is at 0:30, not at 10:00. A blanket *"never smaller"* would be a bet that track, round and shuffle occur once per stream — they come back with every resume.
...
- **Save** upserts by name (case-insensitive). Tapping a playlist's name loads it back into the form, as in the Actions Hub app. Beside it sit **Track order** and **Check URLs** (tests every link), and the **⋮ menu** holds *Edit*, *Move up / down* and *Delete* — three buttons instead of six keep the card inside the screen on a phone.
- *Move up / down* moves a playlist one place, and **Sort A–Z** puts the whole list in alphabetical order once (umlauts sort as their base letter, not behind Z). The order is stored, not just displayed — it is also the order Alexa reads out when she asks which playlist to play, so the bedtime list does not have to come last. A–Z is an action rather than a view setting, so moving a single entry afterwards still works.
- **Track order** (the numbered-list button, active from two tracks on) opens just the track names of one playlist, numbered in play order. Drag a track by its handle (finger or mouse; the page scrolls along at the screen edge) or use its arrows, or sort them by name — the button offers A–Z, then Z–A, and switches back after each press or once a track is moved by hand (numbers sort naturally, so *Folge 10* comes after *Folge 9*), then **Save order** — the buttons stay pinned to the bottom of the screen while the list scrolls, and *Cancel* asks before throwing a new order away. Only the order changes — switches, folder link and track names stay as they are — and a stored *Resume* position moves with its track instead of pointing at whatever now sits at the old number. With shuffle on, the order is kept but only counts once shuffle is off again.
- Blank lines are ignored; anything that is not an `https://` URL is rejected with its line number. A name needs at least one letter or digit — one that cannot be spoken cannot be found either.
- A spoken name that only *contains* a playlist name, or is contained in one, picks the **longest** such name: *"die Kinderlieder"* starts *Kinderlieder*, not a playlist called *Lieder* that happens to stand higher in the list. Up to 200 tracks per playlist.

Everything goes through `/api/manage?type=playlists` (`GET`, `POST {name, urls, wiederholen, ansage, zufall, fortsetzen}` — `fortsetzen` is `"aus"`, `"titel"` or `"sekunde"` rather than a switch, and a stored `true` from before the two gaits still reads as `"sekunde"` —, `DELETE {name}`, `GET &pruefen=1&name=…` for the check, `GET &import=1&url=…` for the folder import, `POST &sortieren=1 {namen: […]}` for the order, `POST &titelSortieren=1 {name, urls: […]}` for the order of the tracks within one playlist, `GET &ton=1` for the monthly traffic total, `GET &verlauf=1` / `DELETE &verlauf=1` to read or clear the playback history), protected by `ADMIN_PASSWORD` like the rest of the dashboard. Leaving a switch out of a `POST` keeps its stored value, so a script that only fixes a track list cannot flip one by omission; switching it off has to arrive as an explicit `false` — so it can be scripted from a workflow just like persons and zones (8.5).
...
**Every start attempt leaves a line**, whether or not the playlist announces itself: `musik-box spielt Udo CD eins (gehoert: "Udo CD 1") ab 1/24 bei 0 ms (…): …`, and a name that matched nothing says so with what it was compared against. Without those, a failed match and a silent box look identical from the outside.
...
**Afterwards:** *"Alexa, weiter"* works even when the Echo has forgotten the stream — after the radio in between, or the next day. Without a running stream the skill falls back on the newest resume mark and carries on with the playlist that was stopped last. Only playlists with *Resume* set leave such a mark, and it is taken up in that playlist's gait. A playlist that was *interrupted* beats one that has played to its end, even a more recent one — the finished one has nothing to carry on; it only starts over when there is nothing else.

**While playing:** *"Alexa, nächster Titel"*, *"voriger Titel"*, *"Pause"*, *"weiter"*, *"von vorn"* and *"Stopp"* work as usual, as do the buttons on Echo Show and in the Alexa app. *"Zufallswiedergabe an"* and *"aus"* reshuffle the running playback without touching the stored playlist — the order lives in the token, so the change reaches exactly this one stream. Neither jumps: the track that is playing carries on at its second. *"An"* puts it first in a fresh random order, so every other track still comes this round; *"aus"* carries on in list order from there. Asking Alexa to repeat, on the other hand, only reports how the running playlist is set and points at the dashboard: flipping that one by voice would change the playlist for good and for everyone. A track that fails to load is **retried once**, picking up where it broke off, and only skipped if it fails again. If three tracks in a row fail to start, or skipping would wrap around to the next round, playback stops instead of circling forever.
...
**And what nobody is waiting for runs outside that budget.** The history is
written once the answer is out. The resume mark is written *before* the answer
(see **Resume**), and for player events it gets a deadline of its own (2 s)
instead of the leftover budget. Sharing the budget with the answer meant losing
to it: on a cold start only the floor of 1200 ms is left,
...
Each request logs its own duration as `musik-box <type> [<intent>] mit|ohne Sitzung in <n> ms`. The box has
...
track `musik-box Ton geliefert: 04 - La-Le-Lu.mp3 <range> → 206, 4.2 MB in <n> ms`. A log full of the
...
IntentRequest PlayPlaylistIntent mit Sitzung in 2705 ms, Alexa wartet seit 8123 ms (Vorlauf 5418 ms)`. Anything
...
| Nothing at all happens after the second sentence | the session had already closed, or the answer arrived too late | should no longer occur — see **Why the skill never goes silent** above; check the `musik-box … ms` line in the Vercel logs |
...
| Silence from a FRITZ!NAS folder playlist, `503` in the log | the monthly ceiling `MUSIK_TON_BUDGET_GB` is used up — the dashboard shows the total under the playlist list, and a click on that line refreshes it | raise it, or wait for the next month; `0` removes the ceiling |
| Something is wrong with a response and nothing says what | `SessionEndedRequest` used to fall into the catch-all empty answer: its `reason` and `error` were thrown away. It is the one place where Alexa says of her own accord why she could not process a response (`INVALID_RESPONSE`, `DEVICE_COMMUNICATION_ERROR`, `INTERNAL_ERROR`) | fixed: a session end that is not `USER_INITIATED`, or that carries an error, is logged and written to the history with its reason and message |
...
| A start plays nothing and nothing at all is logged about it | the Echo dropped the `Play` without a word. Measured on 21 Sept between 21:44 and 21:49: three starts out of twelve, each answered in 12–20 ms, each audio file fetched and delivered in full (`200`, 7.7 MB), and not one `PlaybackStarted`, `PlaybackFailed` or `ExceptionEncountered` afterwards | the history now records it by itself: a start that does not report back within `MUSIK_STARTWACHE` appears as `STUMM (kein PlaybackStarted)` under the time of the start, with the token handed out and the one the device held. The `PlaybackStarted` side of that check is written **before** its (empty) answer, and the start mark carries the time of the request rather than of the write — otherwise a Vercel instance frozen after its answer made a start that played look silent. Note what the same logs also showed: an **identical** token played 7 times out of 7 — it is the stale, foreign or absent device state that goes silent, which is the opposite of what #137 assumed |
| Start a playlist, stop shortly after, start again — and it stays silent | the Echo was paused on exactly that stream and got **the same address** again. The skill answers correctly and in milliseconds, the Echo fetches the file and receives all of it — and does not play it. A different stream token alone was not enough (#137) | every directive now carries its own address (`&n=…` behind the signed token; the endpoint reads only `ton`). The history in the dashboard names the token handed out and the one the device still holds, side by side (`=` between them means identical), so the next occurrence is decidable without a Vercel export. If it still happens, try `MUSIK_CLEAR_QUEUE=0` |
| After stopping and starting again, the remembered second is gone and "continue" begins at the start of the track | `selberTitel` compares the round as well, and #137 counted it up on every restart — so `standNichtZurueck` no longer recognised the stored position and a `PlaybackStarted` with 0 ms overwrote it. In the log it shows as `Stand: 0 ms gemerkt (anfang)` where `… verworfen, … stehen schon` used to be | fixed: the round is no longer counted up on a start. What it was meant to achieve is covered by the fresh address, which is what actually helped |
| The same file is fetched three or four times within a minute, and the monthly total climbs fast | every start fetches the whole track, and `PlaybackNearlyFinished` arrives about a second later and fetches the next one too. Stop and start again and both are ordered afresh — while the earlier deliveries ran on to the end, on the very instance and line the new start needs | superseded deliveries now stop: a start address carries `&g=1` and calls out a new playback, and anything belonging to an older one ends within five seconds (`MUSIK_TON_LAUF=0` switches it off). What Alexa does is not ours to change: the Echo discards its buffer on `REPLACE_ALL` and fetches again |
| A delivery of the same file takes 60–90 s instead of 7–13 s | the Echo was paused mid-download: it stops reading but does not close, so the pass-through keeps pushing the rest — pinned to the instance that must answer Alexa within eight seconds, and billed | a watchdog ends a delivery that takes no bytes at all for `MUSIK_TON_STILLSTAND_MS` (default 60 s). It does **not** catch a delivery that is merely slow, and deliberately so: while bytes are flowing, someone is listening |
...
| The remembered second keeps falling back to the start of a track | four causes, all fixed. Two older ones: a resume that once landed at 0:00 overwrote the mark with its own `PlaybackStarted`, and the write shared the answer's time budget, so on a cold start it skipped itself in silence. Two found in the logs afterwards: the write had been moved *behind* the answer, where Vercel freezes the instance — three writes were 4½ minutes late, expired or never ran — and the guard only fended off an exact zero, while this Echo reports ≈1 s at every track start | the mark is written **before** the answer, with a floor of its own for player events; anything below `MUSIK_VORLAUF_MS` counts as "the start of the track" and no longer overwrites a remembered second; and a write that does not land now reports `NICHT gespeichert` instead of `gemerkt` |
| The dashboard history shows `… ms gemerkt` but resuming still starts at the beginning | the write had given up on its deadline and said nothing; the caller reported success regardless | fixed — the line now reads `NICHT gespeichert`. If you still see it, the database was unreachable, not the resume logic |
| The dashboard history stays empty for minutes after a stop | the history entry is written after the answer, and Vercel freezes the instance there until the next request reaches it | expected, and it does not affect the remembered second any more — that one is written before the answer. Press *Refresh* again a little later |
...
| *"Alexa, frag familien finder, ob das Handy von [Name] klingeln kann"* | **Asks back first**, then makes the phone ring (Mylo app required) |
| *"Alexa, frag familien finder, lass das Handy von [Name] aufhören"* | Stops sound, torch and announcement — no confirmation |
...
| *"Alexa, frag meine plattenkiste nach den playlists"* | Lists the playlists from the dashboard |
