...
| `MUSIK_TON_MAX_MB` | *(optional, Meine Plattenkiste)* How much of a file is fetched from the box per request, in MB. Default `6`, `0` switches the cap off. It exists because a Vercel function has a wall clock (10 s on Hobby without Fluid Compute) — the player asks for the rest with the next range |
...
**Long files are handed over in pieces.** A Vercel function has a wall clock, and how long it may run is set by the plan — ten seconds on a Hobby project without Fluid Compute. The piece has to cross that window twice: from the box up to Vercel (your uplink, the bottleneck) and on to the Echo. The proxy therefore never requests more than `MUSIK_TON_MAX_MB` (default **6**, which is about five seconds at a 10 Mbit/s uplink) and answers `206` with the matching `Content-Range`; the player asks for the rest, exactly as it does after every pause. `MUSIK_TON_MAX_MB=0` switches the cap off.

> **`maxDuration` is deliberately not set in `vercel.json`.** It was, at 60 s, and every call to `/api/skill` then died with `500 FUNCTION_INVOCATION_FAILED` — the build accepts a value the plan does not allow, and the invocation is what fails. The plan's own default applies instead; with Fluid Compute enabled it is 300 s, and only then is a larger `MUSIK_TON_MAX_MB` worth setting.
...
| `500 FUNCTION_INVOCATION_FAILED` on every call to `/api/skill` | `maxDuration` in `vercel.json` was above what the plan allows — the build takes it, the invocation does not | fixed: the setting is gone, the plan's default applies |
| A track stops mid-way, always at the same point | the function's wall clock ran out while passing the bytes through | lower `MUSIK_TON_MAX_MB` (default 6); with Fluid Compute enabled you may instead raise `maxDuration` |
