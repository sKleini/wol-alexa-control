...
| `MUSIK_TON_LANGSAM_MS` | *(optional, Meine Plattenkiste)* From how many milliseconds a single delivery is logged as a warning rather than a note. Default `20000`; the function's wall clock is 60 s |
| `MUSIK_TON_FRIST_MS` | *(optional, Meine Plattenkiste)* How long one fetch may hang at the box before the proxy gives up, in milliseconds. Default `30000` — a measured 17.7 MB chapter took 18.1 s. Below this the response is cut short on purpose: Alexa then reports `PlaybackFailed`, which the skill can recover from, where a hang produces no event at all |
| `MUSIK_TON_ANLAEUFE` | *(optional, Meine Plattenkiste)* How often the proxy picks a torn stream back up before giving up, counting the first fetch. Default `3`; `1` disables the continuation |
| `MUSIK_TON_UHR_MS` | *(optional, Meine Plattenkiste)* What the function's wall clock is assumed to be, in milliseconds — with less than five seconds left, no further attempt is started. Default `60000`; raise it only together with `maxDuration` in `vercel.json` |
...
- **Nothing is queued ahead**, and that is deliberate. `PlaybackNearlyFinished` arrives seconds *after* a track starts; queueing there sends the Echo off to fetch the next track while the current one is still streaming — two fetches at the same box, each through a function with a wall clock. A reported audiobook log showed exactly that: `ENQUEUE`, a parallel fetch, three `Vercel Runtime Timeout Error` and a chapter that died six seconds in. The next track is therefore ordered at `PlaybackFinished`, when the previous fetch is over. The price is a gap of a second or two between tracks; `MUSIK_FRITZ_NAHTLOS=1` takes it back for a box with more headroom. (Every other source keeps the seamless `ENQUEUE` — nothing about those fetches competes.)
...
**A fetch gets a deadline, because a hang is the one failure nobody recovers from.** Measured in production, and it is the run that ended playback:

```
12:40:09  probe starts
12:43:34  TON 206  32768 von 32768 B in 204952 ms
```

Thirty-two kilobytes in three and a half minutes — 160 bytes per second. The box was jammed by a burst of check probes, and the proxy kept pulling at it: the caller had given up after seven seconds, this ran 198 seconds longer. It did not relieve the box, it held it down — and when the next chapter was needed, the box was still jammed. `KOPF_MS` only guards *whether* the box answers; `MUSIK_TON_FRIST_MS` guards whether it finishes. The platform will not do it for you: `vercel.json` sets `maxDuration: 60` and that request ran 205 seconds.

The point is not thrift. **A fast failure is recoverable, a hang is not**: on a short answer Alexa reports `PlaybackFailed`, and `nachFehler` retries once and otherwise moves on; on a hang it reports nothing at all, and the chain — which since #121 hangs solely on `PlaybackFinished` — dies silently. The deadline turns an incurable silence into a case the skill already handles. A run that hits it is logged and marked `aufgegeben` in the history.

**And *Check URLs* no longer touches the box while a folder playlist is playing.** The same run shows why: eight probes one second apart, each dutifully sequential, and the ninth took 205 s. The box needs not just solitude but air between fetches — so the probes are now spaced (`FRITZ_PRUEF_PAUSE_MS`), and while `musik_ton_laeuft` stands the button answers *"playing right now, try again later"* instead of stalling the playlist it was meant to check.

**The dashboard says what happened, so nobody has to read a cloud log.** Three rounds were spent guessing why folder playback stops mid-chapter, because the line that names the cause sat in Vercel's log — and whoever is listening to music does not read that. The same numbers now go into `musik_ton_verlauf` in Redis (the last 40 entries, a few days' expiry) and appear under the traffic counter in the dashboard: per fetch the file, the requested range, the status, **bytes delivered against bytes announced**, the duration, and whether a login at the box happened inside that request; per player event what the Echo reported, with its offset. Read together they name the cause without ambiguity — a short fetch with `ANMELDUNG` beside it is our own login, a short one without is the box, one near 60000 ms is the clock, and a `PlaybackStopped` with no fetch beside it came from the Echo itself. Writing it never costs playback: a Redis that cannot take the entry is a warning in the log and nothing more.
...
**A torn stream is picked back up, and the Echo never notices.** Three things can end a stream mid-chapter: the function's wall clock, the box itself, and a login — which terminates *every* session on the box, the playing track included. In all three cases the player used to see an answer shorter than its own `Content-Length`, play out its buffer and fall silent, while the log called it a success. Now the proxy compares what arrived against what was announced: what is missing is fetched again (`Range: bytes=<delivered>-`) and written into the **same** answer, so the `Content-Length` adds up at the player's end. At most `MUSIK_TON_ANLAEUFE` attempts (default 3), and none within five seconds of `MUSIK_TON_UHR_MS`. Every attempt gets a line — `musik-box Strom fortgesetzt bei …` — and a stream that stays short gets `musik-box Strom abgerissen: 4.2 MB von 19.7 MB`, which tells you next time whether it was the clock (near 60 s), the box (early and irregular) or a login (`FRITZ!NAS-Login ok` right above it).

**And the one cause of its own making is switched off.** While bytes are being written, `musik_ton_laeuft` stands in Redis (TTL 120 s, and only the fetch that set it takes it away). A fetch that sees it takes the remembered session number even past its five-minute window rather than logging in, because a login would end the stream that is playing right now. Only if the box then answers with its login page does the forced second attempt get a fresh number — by then there is nothing left to protect.
...
| A chapter dies seconds after it starts, and the log shows `Vercel Runtime Timeout Error` | two fetches were running at the same box — the playing track and one queued ahead | fixed: a FRITZ!NAS playlist queues nothing ahead. If you set `MUSIK_FRITZ_NAHTLOS=1`, this is the price |
| `musik-box Ton … – langsam; laeuft an der Box noch ein zweiter Abruf?` | one fetch took longer than `MUSIK_TON_LANGSAM_MS` (default 20 s) — the wall clock is 60 s | look for a second `GET /api/skill?ton=…` open at the same time |
...
| Playback stops and the history's newest line is a fetch with a huge `in … ms` | the box was jammed — it dribbled the answer, the proxy waited it out, and the next track then got nothing. A hanging fetch writes no history entry at all (entries are written on completion), which is why the list simply ends | fixed: a fetch that is not done within `MUSIK_TON_FRIST_MS` is given up, marked `AUFGEGEBEN`, and the box is released. Do not press *Check URLs* while a folder playlist plays — it is now blocked for exactly this reason |
| Playback stops mid-chapter and no next track follows | one fetch delivered less than it announced; the Echo then reports `PlaybackStopped`, and since a FRITZ!NAS playlist queues nothing ahead, nothing orders the next track (a `Stopped` may carry no directive) | open the dashboard: the last entries under the traffic counter name the cause. *"Alexa, weiter"* resumes to the second in the meantime |
| Playback stops mid-chapter, by itself | the stream from the box ended early — the wall clock, the box, or a login that ended every session on it | fixed: the proxy fetches the rest and writes it into the same answer. `musik-box Strom fortgesetzt bei …` shows it happening |
| `musik-box Strom abgerissen: … von …` | the rest could not be fetched either, within `MUSIK_TON_ANLAEUFE` attempts and the wall clock | the line names the numbers: close to 60 s is the clock (Fluid Compute, then raise `maxDuration` and `MUSIK_TON_UHR_MS`), early and irregular is the box, a `FRITZ!NAS-Login ok` right above it is a login |
