...
| `POST/GET /api/ring` | Makes a person's phone ring (`?u=<name>`), even in silent mode: sends a Firebase push that the Mylo app turns into an alarm tone. Needs `FCM_PROJECT_ID`, `FCM_CLIENT_EMAIL` and `FCM_PRIVATE_KEY`; without them it answers `{ok:false, reason:"fcm_not_configured"}` so the caller can fall back to its ntfy path. The device token needs no separate registration — Mylo sends it as an `X-Fcm-Token` header on every `/api/location` call |
