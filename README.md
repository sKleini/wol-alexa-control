...
- **Track order** (the numbered-list button, active from two tracks on) opens just the track names of one playlist, numbered in play order. Drag a track by its handle (finger or mouse; the page scrolls along at the screen edge) or use its arrows, or sort them A–Z by name (numbers sort naturally, so *Folge 10* comes after *Folge 9*), then **Save order** — the buttons stay pinned to the bottom of the screen while the list scrolls, and *Cancel* asks before throwing a new order away. Only the order changes — switches, folder link and track names stay as they are — and a stored *Resume* position moves with its track instead of pointing at whatever now sits at the old number. With shuffle on, the order is kept but only counts once shuffle is off again.
