...
- **Track order** (in the menu, active from two tracks on) opens just the track names of one playlist, numbered in play order. Drag a track or use its arrows, or sort them A–Z by name (numbers sort naturally, so *Folge 10* comes after *Folge 9*), then **Save order**. Only the order changes — switches, folder link and track names stay as they are — and a stored *Resume* position moves with its track instead of pointing at whatever now sits at the old number. With shuffle on, the order is kept but only counts once shuffle is off again.
