...
**And then the next run showed that the login is what makes it silent, not the clock.** Seven starts across four logs, with one column added — whether a login ran in that same request, and whether *any* AudioPlayer event followed:

| Alexa waited | login in the request | result |
|---:|:---|:---|
| 1122 ms | no | PlaybackFailed → **PlaybackStarted** |
| 1569 ms | no | **PlaybackStarted** |
| 1737 ms | no | PlaybackFailed → **PlaybackStarted** |
| 1689 ms | **yes** | **nothing** |
| 2003 ms | **yes** | PlaybackFailed |
| 2539 ms | **yes** | **nothing** |
| 2857 ms | **yes** | **nothing** |

Without a login it played three times out of three. With one, three times out of four **no event arrived at all** — the Echo did not even try. And the row at 1689 ms is the one that breaks the timing story: the answer was fast, the number freshly fetched and valid, and it stayed silent anyway. The clock does not separate these cases; the login does.
...
**It logs in only when there is exactly one share.** A login ends every session on the box, so with two folder playlists a cron that logs in would kill them in rotation — A now, B five minutes later and A with it, and so on. With several shares it only keeps alive what is alive; a dead number stays dead until someone actually plays that playlist, and the error path fetches a new one then.

**And the disk stays asleep.** That was why this endpoint was removed in September — the worry that a cron from outside would keep the NAS disk spinning around the clock. It touches no file: `check_nas_rights` is answered from the box's session store. What wakes the disk is the probe before the first note, and that only runs when somebody is actually listening. Unlike the first version, the answer now covers **every** share, one line per share in the log, and never more than the last four digits of a session number — it ends up in a log file, and the number is the key to the share.
...
Only the playlist this request is about gets refreshed — fetching a number costs two calls to the box, and Alexa allows the skill eight seconds. The number is cached in Redis for five minutes — AVM grants ten, extended by every active access. A track the Echo failed to load overtakes that window: `PlaybackFailed` re-checks the number immediately rather than sitting the window out, because an expired one is by far the likeliest cause and the next track would carry the same. If the box cannot be reached at all, the skill **starts anyway** before the first note — see **the login is what makes it silent** below. That reverses an earlier decision, and both halves of it were measured: starting with a stale number used to produce *"Ich spiele das doppelte Lottchen"* and then silence, because the error path of the day could not repair it. It can now, and the login it replaces costs more than it buys.
...
HTTP 206, audio/mpeg, Kopf nach <n> ms, 32 KB Ton`: that is the track itself,
fetched and *read* before anything is promised — see **the disk has to be awake
too** above.
...
| `MEDIA_ERROR_INTERNAL_SERVER_ERROR – Device playback error` on **every** track of a playlist, at `Offset: 0` or `1`, while the skill's own probe reports `HTTP 206, audio/mpeg` right before each one | not the file, the path, the session number or the disk — the skill fetched that exact URL seconds earlier. What differs is the route: the skill goes over IPv4, an Echo prefers IPv6, and a MyFRITZ! name carries both | the probe now reads 32 KB instead of knocking on one byte, the log names both routes (`musik-box Weg zur Box: …`), and after three tracks that do not start the playback ends instead of burning through the list — see **the counter-check itself turned out to be too weak** |
...
| A folder playlist plays one or two tracks and then stops, or stays silent from the second track on, while a **file** share plays fine | the address was minted at `PlaybackNearlyFinished`, seconds after the previous track started, and used minutes later — by then its session number was stale | fixed: for FRITZ!NAS the next track is ordered at `PlaybackFinished` instead. `MUSIK_FRITZ_NAHTLOS=1` restores the old behaviour |
| Switching between two folder playlists logs in every time, and starting the second one cuts off the first | only one session number was remembered for the whole box, so a number belonging to another share went straight to a login | fixed: one number per share, asked about rather than discarded — see **one remembered number per share** |
| `FRITZ!NAS-Sitzung nachgefragt: gilt noch` in the log, and the Echo still does not get the file | the session was alive but belonged to a different share, so it had no rights to that path | fixed: the folder from `check_nas_rights` is compared with the playlist's — look for `Sitzung gehoert zu "…", die Playlist zu "…"` |
